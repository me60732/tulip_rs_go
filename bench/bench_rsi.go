...
func init() {
	register(BenchmarkDef{
		...
		CinarFn: func(s Stock, opts []float64) error {
			// Cinar has RsiPeriod(period int, closing []float64) ([]float64, []float64)
			// This matches our definition: Wilder-smoothed RSI with period parameter.
			// Returns (rsi values, signal line). We consume the first array's first value.
			result, _ := indicator.RsiPeriod(int(opts[0]), s.Close)
			if len(result) == 0 {
