...
func init() {
	register(BenchmarkDef{
		...
		TulipFn: func(s Stock, opts []float64) error {
			res, state, err := indicators.Cvi.Indicator(s.High, s.Low, opts, nil)
			if err != nil {
				return err
			}
			defer res.Close()
		...
		CinarFn: nil, // cinar v1.3.0 has no CVI implementation
		SimdAssetsFn: func(stocks []Stock, opts []float64) error {
			...
			res, err := indicators.Cvi.SimdByAssets(assets, opts, nil)
			if err != nil {
				return err
			}
			defer res.Close()
		...
		SimdOptionsFn: func(s Stock, optionSets [][]float64) error {
			res, err := indicators.Cvi.SimdByOptions(s.High, s.Low, optionSets, nil)
			if err != nil {
				return err
			}
			defer res.Close()
