...
func init() {
	register(BenchmarkDef{
		...
		TulipFn: func(s Stock, opts []float64) error {
			res, state, err := indicators.Hma.Indicator(s.Close, opts, nil)
			if err != nil {
				return err
			}
			defer res.Close()
		...
		SimdAssetsFn: func(stocks []Stock, opts []float64) error {
			...
			for i, s := range stocks {
				assets[i] = [indicators.HmaInputs][]float64{s.Close}
			}
			res, err := indicators.Hma.SimdByAssets(assets, opts, nil)
			if err != nil {
				return err
			}
			defer res.Close()
		...
		SimdOptionsFn: func(s Stock, optsList [][]float64) error {
			res, err := indicators.Hma.SimdByOptions(s.Close, optsList, nil)
			if err != nil {
				return err
			}
			defer res.Close()
