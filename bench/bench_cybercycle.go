...
func init() {
	register(BenchmarkDef{
		Name:    "cybercycle",
		Options: [][]float64{{0.05}, {0.07}, {0.10}, {0.15}}, // matches Python options_list
		TulipFn: func(s Stock, opts []float64) error {
			res, state, err := indicators.Cybercycle.Indicator(s.Close, opts, nil)
			if err != nil {
				return err
			}
			defer res.Close()
		...
		CinarFn: nil, // cinar v1.3.0 has no CyberCycle implementation
		SimdAssetsFn: func(stocks []Stock, opts []float64) error {
			...
			res, err := indicators.Cybercycle.SimdByAssets(assets, opts, nil)
			if err != nil {
				return err
			}
			defer res.Close()
		...
		SimdOptionsFn: func(s Stock, optionSets [][]float64) error {
			res, err := indicators.Cybercycle.SimdByOptions(s.Close, optionSets, nil)
			if err != nil {
				return err
			}
			defer res.Close()
