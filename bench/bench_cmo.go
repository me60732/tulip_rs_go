...
func init() {
	register(BenchmarkDef{
		...
		TulipFn: func(s Stock, opts []float64) error {
			res, state, err := indicators.Cmo.Indicator(s.Close, opts, nil)
			if err != nil {
				return err
			}
			defer res.Close()
		...
		CinarFn: nil, // cinar v1.3.0 has no CMO implementation
		SimdAssetsFn: func(stocks []Stock, opts []float64) error {
			...
			res, err := indicators.Cmo.SimdByAssets(assets, opts, nil)
			if err != nil {
				return err
			}
			defer res.Close()
		...
		SimdOptionsFn: func(s Stock, optionSets [][]float64) error {
			res, err := indicators.Cmo.SimdByOptions(s.Close, optionSets, nil)
			if err != nil {
				return err
			}
			defer res.Close()
