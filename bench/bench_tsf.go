...
func init() {
	register(BenchmarkDef{
		...
		TulipFn: func(s Stock, opts []float64) error {
			res, state, err := indicators.Tsf.Indicator(s.Close, opts, nil)
			if err != nil {
				return err
			}
			defer res.Close()
		...
		CinarFn: nil, // no Cinar equivalent — tsf projects END value via least-squares fit but cinar returns fitted line
		SimdAssetsFn: func(stocks []Stock, opts []float64) error {
			...
			res, err := indicators.Tsf.SimdByAssets(assets, opts, nil)
			if err != nil {
				return err
			}
			defer res.Close()
		...
		SimdOptionsFn: func(s Stock, optionSets [][]float64) error {
			res, err := indicators.Tsf.SimdByOptions(s.Close, optionSets, nil)
			if err != nil {
				return err
			}
			defer res.Close()
