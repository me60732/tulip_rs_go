// Highpass example: full compute, streaming continuation, state persistence
// (serialize/deserialize/clone), and both SIMD modes — the Go mirror of
// tulip_rs_ffi/examples/highpass_example.c, with every step verified.
package main
...
func main() {
	var c demo.Check
	options := []float64{20.0} // period/cutoff

	info := indicators.Highpass.Info()
	...
	n := 2*int(indicators.Highpass.MinData(options)) + 100
	series := demo.SeriesFor(info.Inputs, n)
	real := series[0]

	// ---- full compute -----------------------------------------------------
	fmt.Println("\n=== full calculation (all optional outputs) ===")
	res, st, err := indicators.Highpass.Indicator(real, options, nil)
	...
	if err == nil {
		...
		fullHighpass := append([]float64(nil), tulip.AsFloat64(res.Rows[0])...)
		res.Close()
		st.Close()
		...
		partial := n - 50
		res, st, err := indicators.Highpass.Indicator(real[:partial], options, nil)
		...
		if err == nil {
			...
			c.Err("batch", err)
			if err == nil {
				tail := fullHighpass[len(fullHighpass)-len(br.Rows[0]):]
			...
			c.Err("serialize", err)
			if err == nil {
				fmt.Printf("  bincode blob: %d bytes (indicator id 0x%08x)\n", len(blob), indicators.HighpassID)
			}
			rs, err := indicators.Highpass.DeserializeState(blob)
	...
	// ---- SIMD by assets ---------------------------------------------------
	fmt.Println("\n=== SIMD by assets (N=4) ===")
	assets := [][indicators.HighpassInputs][]float64{
		{real},
		{demo.Scale(real, 1.2)},
	}
	sim, err := indicators.Highpass.SimdByAssets(assets, options, nil)
	c.Err("simd by assets", err)
	if err == nil {
		for i := range sim.Results {
			r, s2, err := indicators.Highpass.Indicator(assets[i][0], options, nil)
	...
	// ---- SIMD by options --------------------------------------------------
	fmt.Println("\n=== SIMD by options (N=4) ===")
	sim2, err := indicators.Highpass.SimdByOptions(real, [][]float64{{10}, {20}, {30}, {50}}, nil)
	c.Err("simd by options", err)
	if err == nil {
		for i, o := range [][]float64{{10}, {20}, {30}, {50}} {
			r, s2, err := indicators.Highpass.Indicator(real, o, nil)
