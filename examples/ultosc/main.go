// ULTOSC example: full compute, streaming continuation, state persistence
// (serialize/deserialize/clone), and both SIMD modes — the Go mirror of
// tulip_rs_ffi/examples/ultosc_example.c, with every step verified.
package main
...
func main() {
	var c demo.Check
	options := []float64{2.0, 3.0, 5.0} // short_period, medium_period, long_period

	info := indicators.Ultosc.Info()
	...
	n := 2*int(indicators.Ultosc.MinData(options)) + 100
	series := demo.SeriesFor(info.Inputs, n)
	high, low, close := series[0], series[1], series[2]

	// ---- full compute -----------------------------------------------------
	fmt.Println("\n=== full calculation ===")
	res, st, err := indicators.Ultosc.Indicator(high, low, close, options, nil)
	...
	if err == nil {
		...
		fullUltosc := append([]float64(nil), tulip.AsFloat64(res.Rows[0])...)
		res.Close()
		st.Close()
		...
		partial := n - 50
		res, st, err := indicators.Ultosc.Indicator(high[:partial], low[:partial], close[:partial], options, nil)
		...
		if err == nil {
			...
			c.Err("batch", err)
			if err == nil {
				tail := fullUltosc[len(fullUltosc)-len(br.Rows[0]):]
			...
			c.Err("serialize", err)
			if err == nil {
				fmt.Printf("  bincode blob: %d bytes (indicator id 0x%08x)\n", len(blob), indicators.UltoscID)
			}
			rs, err := indicators.Ultosc.DeserializeState(blob)
	...
	// ---- SIMD by assets ---------------------------------------------------
	fmt.Println("\n=== SIMD by assets (N=2) ===")
	assets := [][indicators.UltoscInputs][]float64{
		{high, low, close},
		{demo.Scale(high, 1.2), demo.Scale(low, 1.2), demo.Scale(close, 1.2)},
	}
	sim, err := indicators.Ultosc.SimdByAssets(assets, options, nil)
	c.Err("simd by assets", err)
	if err == nil {
		for i := range sim.Results {
			r, s2, err := indicators.Ultosc.Indicator(assets[i][0], assets[i][1], assets[i][2], options, nil)
	...
	// ---- SIMD by options --------------------------------------------------
	fmt.Println("\n=== SIMD by options (N=4) ===")
	// Valid ordering: short_period < medium_period < long_period
	sim2, err := indicators.Ultosc.SimdByOptions(high, low, close,
		[][]float64{{2, 3, 5}, {3, 4, 7}, {5, 7, 10}, {7, 10, 14}}, nil)
	c.Err("simd by options", err)
	if err == nil {
		for i, o := range [][]float64{{2, 3, 5}, {3, 4, 7}, {5, 7, 10}, {7, 10, 14}} {
			r, s2, err := indicators.Ultosc.Indicator(high, low, close, o, nil)
