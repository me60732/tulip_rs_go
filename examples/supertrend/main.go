...
// tulip_rs_ffi/examples/supertrend_example.c, with every step verified.
package main
...
func main() {
	var c demo.Check
	options := []float64{7.0, 3.0} // period, step

	info := indicators.Supertrend.Info()
	...
	n := 2*int(indicators.Supertrend.MinData(options)) + 100
	series := demo.SeriesFor(info.Inputs, n)
	high, low, close := series[0], series[1], series[2]

	// ---- full compute -----------------------------------------------------
	fmt.Println("\n=== full calculation (all optional outputs) ===")
	res, st, err := indicators.Supertrend.Indicator(high, low, close, options, []bool{true, true})
	c.Err("full indicator", err)
	if err == nil {
		for i, name := range append([]string{"supertrend"}, info.OptionalOutputs...) {
			if i < len(res.Rows) {
				fmt.Printf("  %-10s %d values\n", name, len(res.Rows[i]))
			}
		}
		fullSupertrend := append([]float64(nil), tulip.AsFloat64(res.Rows[0])...)
		res.Close()
		st.Close()
		...
		partial := n - 50
		res, st, err := indicators.Supertrend.Indicator(high[:partial], low[:partial], close[:partial], options, nil)
		...
		if err == nil {
			...
			c.Err("batch", err)
			if err == nil {
				tail := fullSupertrend[len(fullSupertrend)-len(br.Rows[0]):]
			...
			c.Err("serialize", err)
			if err == nil {
				fmt.Printf("  bincode blob: %d bytes (indicator id 0x%08x)\n", len(blob), indicators.SupertrendID)
			}
			rs, err := indicators.Supertrend.DeserializeState(blob)
	...
	// ---- SIMD by assets ---------------------------------------------------
	fmt.Println("\n=== SIMD by assets (N=2) ===")
	assets := [][indicators.SupertrendInputs][]float64{
		{high, low, close},
		{demo.Scale(high, 1.2), demo.Scale(low, 1.2), demo.Scale(close, 1.2)},
	}
	sim, err := indicators.Supertrend.SimdByAssets(assets, options, nil)
	c.Err("simd by assets", err)
	if err == nil {
		for i := range sim.Results {
			r, s2, err := indicators.Supertrend.Indicator(assets[i][0], assets[i][1], assets[i][2], options, nil)
	...
	// ---- SIMD by options --------------------------------------------------
	fmt.Println("\n=== SIMD by options (N=4) ===")
	sim2, err := indicators.Supertrend.SimdByOptions(high, low, close, [][]float64{{5.0, 2.0}, {7.0, 3.0}, {10.0, 2.5}, {14.0, 3.0}}, nil)
	c.Err("simd by options", err)
	if err == nil {
		for i, o := range [][]float64{{5.0, 2.0}, {7.0, 3.0}, {10.0, 2.5}, {14.0, 3.0}} {
			r, s2, err := indicators.Supertrend.Indicator(high, low, close, o, nil)
