// NVI example: full compute, streaming continuation, state persistence
// (serialize/deserialize/clone), and SIMD by assets — the Go mirror of
// tulip_rs_ffi/examples/nvi_example.c, with every step verified.
package main
...
func main() {
	var c demo.Check
	options := []float64{} // zero options

	info := indicators.Nvi.Info()
	...
	n := 2*int(indicators.Nvi.MinData(options)) + 100
	series := demo.SeriesFor(info.Inputs, n)
	real, volume := series[0], series[1]

	// ---- full compute -----------------------------------------------------
	fmt.Println("\n=== full calculation ===")
	res, st, err := indicators.Nvi.Indicator(real, volume, options, nil)
	...
	if err == nil {
		...
		fullNvi := append([]float64(nil), tulip.AsFloat64(res.Rows[0])...)
		res.Close()
		st.Close()
		...
		partial := n - 50
		res, st, err := indicators.Nvi.Indicator(real[:partial], volume[:partial], options, nil)
		c.Err("partial indicator", err)
		if err == nil {
			res.Close()
			br, err := st.Batch(real[partial:], volume[partial:], nil)
			c.Err("batch", err)
			if err == nil {
				tail := fullNvi[len(fullNvi)-len(br.Rows[0]):]
			...
			c.Err("serialize", err)
			if err == nil {
				fmt.Printf("  bincode blob: %d bytes (indicator id 0x%08x)\n", len(blob), indicators.NviID)
			}
			rs, err := indicators.Nvi.DeserializeState(blob)
			c.Err("deserialize", err)
			cl, err := st.Clone()
			c.Err("clone", err)

			b1, e1 := st.Batch(real[partial:], volume[partial:], nil)
			b2, e2 := rs.Batch(real[partial:], volume[partial:], nil)
			b3, e3 := cl.Batch(real[partial:], volume[partial:], nil)
	...
	assets := [][indicators.NviInputs][]float64{
		{real, volume},
		{demo.Scale(real, 1.2), demo.Scale(volume, 1.2)},
	}
	sim, err := indicators.Nvi.SimdByAssets(assets, options, nil)
	c.Err("simd by assets", err)
	if err == nil {
		for i := range sim.Results {
			r, s2, err := indicators.Nvi.Indicator(assets[i][0], assets[i][1], options, nil)
	...
	// ---- SIMD by options --------------------------------------------------
	fmt.Println("\n=== SIMD by options (N/A) ===")
	fmt.Println("  NVI has zero options, so SimdByOptions is not available.")
	_, err2 := indicators.Nvi.SimdByOptions(real, volume, [][]float64{{}}, nil)
	c.Match("SimdByOptions returns expected error", err2 != nil)
