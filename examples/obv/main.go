...
// tulip_rs_ffi/examples/obv_example.c, with every step verified.
package main
...
func main() {
	...
	n := 2*int(indicators.Obv.MinData(options)) + 100
	series := demo.SeriesFor(info.Inputs, n)
	real, volume := series[0], series[1]

	// ---- full compute -----------------------------------------------------
	fmt.Println("\n=== full calculation ===")
	res, st, err := indicators.Obv.Indicator(real, volume, options, nil)
	...
	if err == nil {
		...
		fullObv := append([]float64(nil), tulip.AsFloat64(res.Rows[0])...)
		res.Close()
		st.Close()
		...
		partial := n - 50
		res, st, err := indicators.Obv.Indicator(real[:partial], volume[:partial], options, nil)
		...
		if err == nil {
			...
			c.Err("batch", err)
			if err == nil {
				tail := fullObv[len(fullObv)-len(br.Rows[0]):]
			...
			c.Err("serialize", err)
			if err == nil {
				fmt.Printf("  bincode blob: %d bytes (indicator id 0x%08x)\n", len(blob), indicators.ObvID)
			}
			rs, err := indicators.Obv.DeserializeState(blob)
	...
	sim, err := indicators.Obv.SimdByAssets(assets, options, nil)
	c.Err("simd by assets", err)
	if err == nil {
		for i := range sim.Results {
			r, s2, err := indicators.Obv.Indicator(assets[i][0], assets[i][1], options, nil)
	...
	// ---- SIMD by options --------------------------------------------------
	fmt.Println("\n=== SIMD by options (N/A) ===")
	fmt.Println("  OBV has zero options, so SimdByOptions is not available.")
	_, err2 := indicators.Obv.SimdByOptions(real, volume, [][]float64{{}}, nil)
