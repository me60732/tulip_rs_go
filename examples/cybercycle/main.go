// Cybercycle example: full compute, streaming continuation, state persistence
// (serialize/deserialize/clone), and both SIMD modes — the Go mirror of
// tulip_rs_ffi/examples/cybercycle_example.c, with every step verified.
package main
...
func main() {
	...
	info := indicators.Cybercycle.Info()
	...
	n := 2*int(indicators.Cybercycle.MinData(options)) + 100
	series := demo.SeriesFor(info.Inputs, n)
	real := series[0]

	// ---- full compute -----------------------------------------------------
	fmt.Println("\n=== full calculation ===")
	res, st, err := indicators.Cybercycle.Indicator(real, options, nil)
	c.Err("full indicator", err)
	if err == nil {
		for i, name := range append([]string{"cybercycle_line"}, info.OptionalOutputs...) {
			if i < len(res.Rows) {
				fmt.Printf("  %-6s %d values\n", name, len(res.Rows[i]))
			}
		}
		fullCycle := append([]float64(nil), tulip.AsFloat64(res.Rows[0])...)
		res.Close()
		st.Close()
		...
		partial := n - 50
		res, st, err := indicators.Cybercycle.Indicator(real[:partial], options, nil)
		...
		if err == nil {
			...
			c.Err("batch", err)
			if err == nil {
				tail := fullCycle[len(fullCycle)-len(br.Rows[0]):]
			...
			c.Err("serialize", err)
			if err == nil {
				fmt.Printf("  bincode blob: %d bytes (indicator id 0x%08x)\n", len(blob), indicators.CybercycleID)
			}
			rs, err := indicators.Cybercycle.DeserializeState(blob)
	...
	// ---- SIMD by assets ---------------------------------------------------
	fmt.Println("\n=== SIMD by assets (N=2) ===")
	assets := [][indicators.CybercycleInputs][]float64{
		{real},
		{demo.Scale(real, 1.2)},
	}
	sim, err := indicators.Cybercycle.SimdByAssets(assets, options, nil)
	c.Err("simd by assets", err)
	if err == nil {
		for i := range sim.Results {
			r, s2, err := indicators.Cybercycle.Indicator(assets[i][0], options, nil)
	...
	// ---- SIMD by options --------------------------------------------------
	fmt.Println("\n=== SIMD by options (N=4) ===")
	sim2, err := indicators.Cybercycle.SimdByOptions(real, [][]float64{{0.05}, {0.07}, {0.10}, {0.15}}, nil)
	c.Err("simd by options", err)
	if err == nil {
		for i, o := range [][]float64{{0.05}, {0.07}, {0.10}, {0.15}} {
			r, s2, err := indicators.Cybercycle.Indicator(real, o, nil)
