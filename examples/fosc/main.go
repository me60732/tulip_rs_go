...
// tulip_rs_ffi/examples/fosc_example.c, with every step verified.
package main
...
func main() {
	...
	n := 2*int(indicators.Fosc.MinData(options)) + 100
	series := demo.SeriesFor(info.Inputs, n)
	real := series[0]

	// ---- full compute -----------------------------------------------------
	fmt.Println("\n=== full calculation (all optional outputs) ===")
	res, st, err := indicators.Fosc.Indicator(real, options, []bool{true, true, true, true})
	c.Err("full indicator", err)
	if err == nil {
		for i, name := range append([]string{"fosc"}, info.OptionalOutputs...) {
			if i < len(res.Rows) {
				fmt.Printf("  %-12s %d values\n", name, len(res.Rows[i]))
			}
		}
		fullFosc := append([]float64(nil), tulip.AsFloat64(res.Rows[0])...)
		res.Close()
		st.Close()
		...
		partial := n - 50
		res, st, err := indicators.Fosc.Indicator(real[:partial], options, nil)
		...
		if err == nil {
			...
			c.Err("batch", err)
			if err == nil {
				tail := fullFosc[len(fullFosc)-len(br.Rows[0]):]
			...
			c.Err("serialize", err)
			if err == nil {
				fmt.Printf("  bincode blob: %d bytes (indicator id 0x%08x)\n", len(blob), indicators.FoscID)
			}
			rs, err := indicators.Fosc.DeserializeState(blob)
	...
	// ---- SIMD by assets ---------------------------------------------------
	fmt.Println("\n=== SIMD by assets (N=4) ===")
	assets := [][indicators.FoscInputs][]float64{
		{real},
		{demo.Scale(real, 1.2)},
	}
	sim, err := indicators.Fosc.SimdByAssets(assets, options, nil)
	c.Err("simd by assets", err)
	if err == nil {
		for i := range sim.Results {
			r, s2, err := indicators.Fosc.Indicator(assets[i][0], options, nil)
	...
	// ---- SIMD by options --------------------------------------------------
	fmt.Println("\n=== SIMD by options (N=4) ===")
	sim2, err := indicators.Fosc.SimdByOptions(real, [][]float64{{3}, {5}, {7}, {10}}, nil)
	c.Err("simd by options", err)
	if err == nil {
		for i, o := range [][]float64{{3}, {5}, {7}, {10}} {
			r, s2, err := indicators.Fosc.Indicator(real, o, nil)
