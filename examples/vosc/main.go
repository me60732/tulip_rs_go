// VOSC example: full compute, streaming continuation, state persistence
// (serialize/deserialize/clone), and both SIMD modes — the Go mirror of
// tulip_rs_ffi/examples/vosc_example.c, with every step verified.
package main
...
func main() {
	...
	n := 2*int(indicators.Vosc.MinData(options)) + 100
	series := demo.SeriesFor(info.Inputs, n)
	volume := series[0]

	// ---- full compute -----------------------------------------------------
	fmt.Println("\n=== full calculation (all optional outputs) ===")
	res, st, err := indicators.Vosc.Indicator(volume, options, []bool{true, true})
	c.Err("full indicator", err)
	if err == nil {
		for i, name := range append([]string{"vosc"}, info.OptionalOutputs...) {
			if i < len(res.Rows) {
				fmt.Printf("  %-6s %d values\n", name, len(res.Rows[i]))
			}
		}
		fullVosc := append([]float64(nil), tulip.AsFloat64(res.Rows[0])...)
		res.Close()
		st.Close()
		...
		partial := n - 50
		res, st, err := indicators.Vosc.Indicator(volume[:partial], options, nil)
		c.Err("partial indicator", err)
		if err == nil {
			res.Close()
			br, err := st.Batch(volume[partial:], nil)
			c.Err("batch", err)
			if err == nil {
				tail := fullVosc[len(fullVosc)-len(br.Rows[0]):]
			...
			c.Err("serialize", err)
			if err == nil {
				fmt.Printf("  bincode blob: %d bytes (indicator id 0x%08x)\n", len(blob), indicators.VoscID)
			}
			rs, err := indicators.Vosc.DeserializeState(blob)
			c.Err("deserialize", err)
			cl, err := st.Clone()
			c.Err("clone", err)

			b1, e1 := st.Batch(volume[partial:], nil)
			b2, e2 := rs.Batch(volume[partial:], nil)
			b3, e3 := cl.Batch(volume[partial:], nil)
	...
	assets := [][indicators.VoscInputs][]float64{
		{volume},
		{demo.Scale(volume, 1.2)},
	}
	sim, err := indicators.Vosc.SimdByAssets(assets, options, nil)
	c.Err("simd by assets", err)
	if err == nil {
		for i := range sim.Results {
			r, s2, err := indicators.Vosc.Indicator(assets[i][0], options, nil)
	...
	// ---- SIMD by options --------------------------------------------------
	fmt.Println("\n=== SIMD by options (N=4) ===")
	sim2, err := indicators.Vosc.SimdByOptions(volume, [][]float64{{3.0, 7.0}, {5.0, 10.0}, {8.0, 15.0}, {10.0, 20.0}}, nil)
	c.Err("simd by options", err)
	if err == nil {
		for i, o := range [][]float64{{3.0, 7.0}, {5.0, 10.0}, {8.0, 15.0}, {10.0, 20.0}} {
			r, s2, err := indicators.Vosc.Indicator(volume, o, nil)
