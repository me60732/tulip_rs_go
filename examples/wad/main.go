// WAD example: full compute, streaming continuation, state persistence
// (serialize/deserialize/clone), and SimdByAssets. SimdByOptions is skipped
// because WAD has 0 options.
package main
...
func main() {
	...
	n := 2*int(indicators.Wad.MinData(options)) + 100
	series := demo.SeriesFor(info.Inputs, n)
	high, low, close := series[0], series[1], series[2]

	// ---- full compute -----------------------------------------------------
	fmt.Println("\n=== full calculation (all optional outputs) ===")
	res, st, err := indicators.Wad.Indicator(high, low, close, options, nil)
	c.Err("full indicator", err)
	if err == nil {
		for i, name := range append([]string{"wad"}, info.OptionalOutputs...) {
			if i < len(res.Rows) {
				fmt.Printf("  %-6s %d values\n", name, len(res.Rows[i]))
			}
		}
		fullWad := append([]float64(nil), tulip.AsFloat64(res.Rows[0])...)
		res.Close()
		st.Close()
		...
		partial := n - 50
		res, st, err := indicators.Wad.Indicator(high[:partial], low[:partial], close[:partial], options, nil)
		...
		if err == nil {
			...
			c.Err("batch", err)
			if err == nil {
				tail := fullWad[len(fullWad)-len(br.Rows[0]):]
			...
			c.Err("serialize", err)
			if err == nil {
				fmt.Printf("  bincode blob: %d bytes (indicator id 0x%08x)\n", len(blob), indicators.WadID)
			}
			rs, err := indicators.Wad.DeserializeState(blob)
	...
	sim, err := indicators.Wad.SimdByAssets(assets, options, nil)
	c.Err("simd by assets", err)
	if err == nil {
		for i := range sim.Results {
			r, s2, err := indicators.Wad.Indicator(assets[i][0], assets[i][1], assets[i][2], options, nil)
