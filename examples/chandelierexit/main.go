...
// tulip_rs_ffi/examples/chandelierexit_example.c, with every step verified.
package main
...
func main() {
	var c demo.Check
	options := []float64{5.0, 3.0} // period, multiplier

	info := indicators.Chandelierexit.Info()
	...
	n := 2*int(indicators.Chandelierexit.MinData(options)) + 100
	series := demo.SeriesFor(info.Inputs, n)
	high, low, close := series[0], series[1], series[2]

	// ---- full compute -----------------------------------------------------
	fmt.Println("\n=== full calculation (all optional outputs) ===")
	res, st, err := indicators.Chandelierexit.Indicator(high, low, close, options, []bool{true, true, true})
	c.Err("full indicator", err)
	if err == nil {
		for i, name := range append([]string{"long", "short"}, info.OptionalOutputs...) {
			if i < len(res.Rows) {
				fmt.Printf("  %-6s %d values\n", name, len(res.Rows[i]))
			}
		}
		fullLong := append([]float64(nil), tulip.AsFloat64(res.Rows[0])...)
		res.Close()
		st.Close()
		...
		partial := n - 50
		res, st, err := indicators.Chandelierexit.Indicator(high[:partial], low[:partial], close[:partial], options, nil)
		...
		if err == nil {
			...
			c.Err("batch", err)
			if err == nil {
				tail := fullLong[len(fullLong)-len(br.Rows[0]):]
			...
			c.Err("serialize", err)
			if err == nil {
				fmt.Printf("  bincode blob: %d bytes (indicator id 0x%08x)\n", len(blob), indicators.ChandelierexitID)
			}
			rs, err := indicators.Chandelierexit.DeserializeState(blob)
	...
	// ---- SIMD by assets ---------------------------------------------------
	fmt.Println("\n=== SIMD by assets (N=2) ===")
	assets := [][indicators.ChandelierexitInputs][]float64{
		{high, low, close},
		{demo.Scale(high, 1.2), demo.Scale(low, 1.2), demo.Scale(close, 1.2)},
	}
	sim, err := indicators.Chandelierexit.SimdByAssets(assets, options, nil)
	c.Err("simd by assets", err)
	if err == nil {
		for i := range sim.Results {
			r, s2, err := indicators.Chandelierexit.Indicator(assets[i][0], assets[i][1], assets[i][2], options, nil)
	...
	// ---- SIMD by options --------------------------------------------------
	fmt.Println("\n=== SIMD by options (N=4) ===")
	sim2, err := indicators.Chandelierexit.SimdByOptions(high, low, close, [][]float64{{3.0, 2.0}, {5.0, 3.0}, {7.0, 2.0}, {10.0, 3.0}}, nil)
	c.Err("simd by options", err)
	if err == nil {
		for i, o := range [][]float64{{3.0, 2.0}, {5.0, 3.0}, {7.0, 2.0}, {10.0, 3.0}} {
			r, s2, err := indicators.Chandelierexit.Indicator(high, low, close, o, nil)
