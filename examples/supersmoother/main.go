// Super Smoother Filter example: full compute, streaming continuation, state persistence
// (serialize/deserialize/clone), and both SIMD modes — the Go mirror of
// tulip_rs_ffi/examples/supersmoother_example.c, with every step verified.
package main
...
func main() {
	...
	info := indicators.Supersmoother.Info()
	...
	n := 2*int(indicators.Supersmoother.MinData(options)) + 100
	series := demo.SeriesFor(info.Inputs, n)
	real := series[0]

	// ---- full compute -----------------------------------------------------
	fmt.Println("\n=== full calculation ===")
	res, st, err := indicators.Supersmoother.Indicator(real, options, nil)
	...
	if err == nil {
		...
		fullSupersmoother := append([]float64(nil), tulip.AsFloat64(res.Rows[0])...)
		res.Close()
		st.Close()
		...
		partial := n - 50
		res, st, err := indicators.Supersmoother.Indicator(real[:partial], options, nil)
		...
		if err == nil {
			...
			c.Err("batch", err)
			if err == nil {
				tail := fullSupersmoother[len(fullSupersmoother)-len(br.Rows[0]):]
			...
			c.Err("serialize", err)
			if err == nil {
				fmt.Printf("  bincode blob: %d bytes (indicator id 0x%08x)\n", len(blob), indicators.SupersmootherID)
			}
			rs, err := indicators.Supersmoother.DeserializeState(blob)
	...
	sim, err := indicators.Supersmoother.SimdByAssets(assets, options, nil)
	c.Err("simd by assets", err)
	if err == nil {
		for i := range sim.Results {
			r, s2, err := indicators.Supersmoother.Indicator(assets[i][0], options, nil)
	...
	// ---- SIMD by options --------------------------------------------------
	fmt.Println("\n=== SIMD by options (N=4) ===")
	sim2, err := indicators.Supersmoother.SimdByOptions(real, [][]float64{{5}, {10}, {15}, {20}}, nil)
	c.Err("simd by options", err)
	if err == nil {
		for i, o := range [][]float64{{5}, {10}, {15}, {20}} {
			r, s2, err := indicators.Supersmoother.Indicator(real, o, nil)
