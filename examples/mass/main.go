// Mass example: full compute, streaming continuation, state persistence
// (serialize/deserialize/clone), and both SIMD modes — the Go mirror of
// tulip_rs_ffi/examples/mass_example.c, with every step verified.
package main
...
func main() {
	...
	n := 2*int(indicators.Mass.MinData(options)) + 100
	series := demo.SeriesFor(info.Inputs, n)
	high, low := series[0], series[1]

	// ---- full compute -----------------------------------------------------
	fmt.Println("\n=== full calculation ===")
	res, st, err := indicators.Mass.Indicator(high, low, options, nil)
	...
	if err == nil {
		...
		fullMass := append([]float64(nil), tulip.AsFloat64(res.Rows[0])...)
		res.Close()
		st.Close()
		...
		partial := n - 50
		res, st, err := indicators.Mass.Indicator(high[:partial], low[:partial], options, nil)
		...
		if err == nil {
			...
			c.Err("batch", err)
			if err == nil {
				tail := fullMass[len(fullMass)-len(br.Rows[0]):]
			...
			c.Err("serialize", err)
			if err == nil {
				fmt.Printf("  bincode blob: %d bytes (indicator id 0x%08x)\n", len(blob), indicators.MassID)
			}
			rs, err := indicators.Mass.DeserializeState(blob)
	...
	// ---- SIMD by assets ---------------------------------------------------
	fmt.Println("\n=== SIMD by assets (N=4) ===")
	assets := [][indicators.MassInputs][]float64{
		{high, low},
		{demo.Scale(high, 1.2), demo.Scale(low, 1.2)},
	}
	sim, err := indicators.Mass.SimdByAssets(assets, options, nil)
	c.Err("simd by assets", err)
	if err == nil {
		for i := range sim.Results {
			r, s2, err := indicators.Mass.Indicator(assets[i][0], assets[i][1], options, nil)
	...
	// ---- SIMD by options --------------------------------------------------
	fmt.Println("\n=== SIMD by options (N=4) ===")
	sim2, err := indicators.Mass.SimdByOptions(high, low, [][]float64{{10}, {25}, {50}, {100}}, nil)
	c.Err("simd by options", err)
	if err == nil {
		for i, o := range [][]float64{{10}, {25}, {50}, {100}} {
			r, s2, err := indicators.Mass.Indicator(high, low, o, nil)
