// AO example: full compute, streaming continuation, state persistence
// (serialize/deserialize/clone), and SIMD by assets — the Go mirror of
// tulip_rs_ffi/examples/ao_example.c, with every step verified.
package main
...
func main() {
	...
	if err == nil {
		...
		fullAo := append([]float64(nil), tulip.AsFloat64(res.Rows[0])...)
		res.Close()
		st.Close()
		...
		partial := n - 50
		res, st, err := indicators.Ao.Indicator(high[:partial], low[:partial], options, nil)
		...
		if err == nil {
			...
			c.Err("batch", err)
			if err == nil {
				tail := fullAo[len(fullAo)-len(br.Rows[0]):]
			...
			c.Err("serialize", err)
			if err == nil {
				fmt.Printf("  bincode blob: %d bytes (indicator id 0x%08x)\n", len(blob), indicators.AoID)
			}
			rs, err := indicators.Ao.DeserializeState(blob)
	...
	// ---- SIMD by assets ---------------------------------------------------
	fmt.Println("\n=== SIMD by assets (N=2) ===")
	assets := [][indicators.AoInputs][]float64{
		{high, low},
		{demo.Scale(high, 1.2), demo.Scale(low, 1.2)},
	}
	sim, err := indicators.Ao.SimdByAssets(assets, options, nil)
	c.Err("simd by assets", err)
	if err == nil {
		for i := range sim.Results {
			r, s2, err := indicators.Ao.Indicator(assets[i][0], assets[i][1], options, nil)
