// RoofingFilter example: full compute, streaming continuation, state persistence
// (serialize/deserialize/clone), and both SIMD modes — the Go mirror of
// tulip_rs_ffi/examples/roofingfilter_example.c, with every step verified.
package main
...
func main() {
	...
	info := indicators.Roofingfilter.Info()
	...
	n := 2*int(indicators.Roofingfilter.MinData(options)) + 100
	series := demo.SeriesFor(info.Inputs, n)
	real := series[0]

	// ---- full compute -----------------------------------------------------
	fmt.Println("\n=== full calculation (all optional outputs) ===")
	res, st, err := indicators.Roofingfilter.Indicator(real, options, []bool{})
	c.Err("full indicator", err)
	if err == nil {
		for i, name := range append([]string{"roofingfilter"}, info.OptionalOutputs...) {
			if i < len(res.Rows) {
				fmt.Printf("  %-6s %d values\n", name, len(res.Rows[i]))
			}
		}
		fullRoofing := append([]float64(nil), tulip.AsFloat64(res.Rows[0])...)
		res.Close()
		st.Close()
		...
		partial := n - 50
		res, st, err := indicators.Roofingfilter.Indicator(real[:partial], options, nil)
		...
		if err == nil {
			...
			c.Err("batch", err)
			if err == nil {
				tail := fullRoofing[len(fullRoofing)-len(br.Rows[0]):]
			...
			c.Err("serialize", err)
			if err == nil {
				fmt.Printf("  bincode blob: %d bytes (indicator id 0x%08x)\n", len(blob), indicators.RoofingfilterID)
			}
			rs, err := indicators.Roofingfilter.DeserializeState(blob)
	...
	sim, err := indicators.Roofingfilter.SimdByAssets(assets, options, nil)
	c.Err("simd by assets", err)
	if err == nil {
		for i := range sim.Results {
			r, s2, err := indicators.Roofingfilter.Indicator(assets[i][0], options, nil)
	...
	// ---- SIMD by options --------------------------------------------------
	fmt.Println("\n=== SIMD by options (N=4) ===")
	sim2, err := indicators.Roofingfilter.SimdByOptions(real, [][]float64{{5, 10}, {10, 20}, {15, 30}, {20, 40}}, nil)
	c.Err("simd by options", err)
	if err == nil {
		for i, o := range [][]float64{{5, 10}, {10, 20}, {15, 30}, {20, 40}} {
			r, s2, err := indicators.Roofingfilter.Indicator(real, o, nil)
