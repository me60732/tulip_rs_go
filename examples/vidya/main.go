// VIDYA example: full compute, streaming continuation, state persistence
// (serialize/deserialize/clone), and both SIMD modes — the Go mirror of
// tulip_rs_ffi/examples/vidya_example.c, with every step verified.
package main
...
func main() {
	...
	n := 2*int(indicators.Vidya.MinData(options)) + 100
	series := demo.SeriesFor(info.Inputs, n)
	real := series[0]

	// ---- full compute -----------------------------------------------------
	fmt.Println("\n=== full calculation (all optional outputs) ===")
	res, st, err := indicators.Vidya.Indicator(real, options, []bool{})
	c.Err("full indicator", err)
	if err == nil {
		for i, name := range append([]string{"vidya"}, info.OptionalOutputs...) {
			if i < len(res.Rows) {
				fmt.Printf("  %-6s %d values\n", name, len(res.Rows[i]))
			}
		}
		fullVidya := append([]float64(nil), tulip.AsFloat64(res.Rows[0])...)
		res.Close()
		st.Close()
		...
		partial := n - 50
		res, st, err := indicators.Vidya.Indicator(real[:partial], options, nil)
		...
		if err == nil {
			...
			c.Err("batch", err)
			if err == nil {
				tail := fullVidya[len(fullVidya)-len(br.Rows[0]):]
			...
			c.Err("serialize", err)
			if err == nil {
				fmt.Printf("  bincode blob: %d bytes (indicator id 0x%08x)\n", len(blob), indicators.VidyaID)
			}
			rs, err := indicators.Vidya.DeserializeState(blob)
	...
	// ---- SIMD by assets ---------------------------------------------------
	fmt.Println("\n=== SIMD by assets (N=4) ===")
	assets := [][indicators.VidyaInputs][]float64{
		{real},
		{demo.Scale(real, 1.2)},
	}
	sim, err := indicators.Vidya.SimdByAssets(assets, options, nil)
	c.Err("simd by assets", err)
	if err == nil {
		for i := range sim.Results {
			r, s2, err := indicators.Vidya.Indicator(assets[i][0], options, nil)
	...
	// ---- SIMD by options --------------------------------------------------
	fmt.Println("\n=== SIMD by options (N=4) ===")
	sim2, err := indicators.Vidya.SimdByOptions(real, [][]float64{{3.0, 7.0, 0.1}, {5.0, 10.0, 0.2}, {8.0, 15.0, 0.3}, {10.0, 20.0, 0.5}}, nil)
	c.Err("simd by options", err)
	if err == nil {
		for i, o := range [][]float64{{3.0, 7.0, 0.1}, {5.0, 10.0, 0.2}, {8.0, 15.0, 0.3}, {10.0, 20.0, 0.5}} {
			r, s2, err := indicators.Vidya.Indicator(real, o, nil)
