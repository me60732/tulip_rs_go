// AD example: full compute, streaming continuation, state persistence
// (serialize/deserialize/clone), and both SIMD modes — the Go mirror of
// tulip_rs_ffi/examples/ad_example.c, with every step verified.
package main
...
func main() {
	...
	n := 2*int(indicators.Ad.MinData(options)) + 100
	...
	res, st, err := indicators.Ad.Indicator(high, low, close, volume, options, nil)
	...
	if err == nil {
		...
		fullAd := append([]float64(nil), tulip.AsFloat64(res.Rows[0])...)
		res.Close()
		st.Close()
		...
		partial := n - 50
		res, st, err := indicators.Ad.Indicator(high[:partial], low[:partial], close[:partial], volume[:partial], options, nil)
		...
		if err == nil {
			...
			c.Err("batch", err)
			if err == nil {
				tail := fullAd[len(fullAd)-len(br.Rows[0]):]
			...
			c.Err("serialize", err)
			if err == nil {
				fmt.Printf("  bincode blob: %d bytes (indicator id 0x%08x)\n", len(blob), indicators.AdID)
			}
			rs, err := indicators.Ad.DeserializeState(blob)
	...
	sim, err := indicators.Ad.SimdByAssets(assets, options, nil)
	c.Err("simd by assets", err)
	if err == nil {
		for i := range sim.Results {
			r, s2, err := indicators.Ad.Indicator(assets[i][0], assets[i][1], assets[i][2], assets[i][3], options, nil)
			c.Err("individual asset", err)
			if err == nil {
				// SIMD vs scalar legitimately differ by ulps here (the core
				// ad tests themselves allow approx_eq): compare with tolerance.
