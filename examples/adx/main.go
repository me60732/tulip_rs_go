// ADX example: full compute, streaming continuation, state persistence
// (serialize/deserialize/clone), and both SIMD modes — the Go mirror of
// tulip_rs_ffi/examples/adx_example.c, with every step verified.
package main
...
func main() {
	...
	n := 2*int(indicators.Adx.MinData(options)) + 100
	...
	if err == nil {
		for i, name := range append([]string{"adx"}, info.OptionalOutputs...) {
			if i < len(res.Rows) {
				fmt.Printf("  %-6s %d values\n", name, len(res.Rows[i]))
			}
		}
		fullAdx := append([]float64(nil), tulip.AsFloat64(res.Rows[0])...)
		res.Close()
		st.Close()
		...
		partial := n - 50
		res, st, err := indicators.Adx.Indicator(high[:partial], low[:partial], close[:partial], options, nil)
	...
	sim, err := indicators.Adx.SimdByAssets(assets, options, nil)
	c.Err("simd by assets", err)
	if err == nil {
		for i := range sim.Results {
			r, s2, err := indicators.Adx.Indicator(assets[i][0], assets[i][1], assets[i][2], options, nil)
	...
	// ---- SIMD by options --------------------------------------------------
	fmt.Println("\n=== SIMD by options (N=4) ===")
	sim2, err := indicators.Adx.SimdByOptions(high, low, close, [][]float64{{3}, {5}, {7}, {10}}, nil)
	c.Err("simd by options", err)
	if err == nil {
		for i, o := range [][]float64{{3}, {5}, {7}, {10}} {
			r, s2, err := indicators.Adx.Indicator(high, low, close, o, nil)
