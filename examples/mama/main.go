...
// tulip_rs_ffi/examples/mama_example.c, with every step verified.
package main
...
func main() {
	var c demo.Check
	options := []float64{0.5, 0.05} // fastlimit,slowlimit

	info := indicators.Mama.Info()
	...
	n := 2*int(indicators.Mama.MinData(options)) + 100
	series := demo.SeriesFor(info.Inputs, n)
	real := series[0]

	// ---- full compute -----------------------------------------------------
	fmt.Println("\n=== full calculation (all optional outputs) ===")
	res, st, err := indicators.Mama.Indicator(real, options, []bool{true, true})
	c.Err("full indicator", err)
	if err == nil {
		for i, name := range append([]string{"mama"}, info.OptionalOutputs...) {
			if i < len(res.Rows) {
				fmt.Printf("  %-6s %d values\n", name, len(res.Rows[i]))
			}
		}
		fullMama := append([]float64(nil), tulip.AsFloat64(res.Rows[0])...)
		res.Close()
		st.Close()
		...
		partial := n - 50
		res, st, err := indicators.Mama.Indicator(real[:partial], options, nil)
		...
		if err == nil {
			...
			c.Err("batch", err)
			if err == nil {
				tail := fullMama[len(fullMama)-len(br.Rows[0]):]
			...
			c.Err("serialize", err)
			if err == nil {
				fmt.Printf("  bincode blob: %d bytes (indicator id 0x%08x)\n", len(blob), indicators.MamaID)
			}
			rs, err := indicators.Mama.DeserializeState(blob)
	...
	sim, err := indicators.Mama.SimdByAssets(assets, options, nil)
	c.Err("simd by assets", err)
	if err == nil {
		for i := range sim.Results {
			r, s2, err := indicators.Mama.Indicator(assets[i][0], options, nil)
	...
	// ---- SIMD by options --------------------------------------------------
	fmt.Println("\n=== SIMD by options (N=4) ===")
	sim2, err := indicators.Mama.SimdByOptions(real, [][]float64{{0.3, 0.03}, {0.5, 0.05}, {0.7, 0.07}, {0.9, 0.09}}, nil)
	c.Err("simd by options", err)
	if err == nil {
		for i, o := range [][]float64{{0.3, 0.03}, {0.5, 0.05}, {0.7, 0.07}, {0.9, 0.09}} {
			r, s2, err := indicators.Mama.Indicator(real, o, nil)
