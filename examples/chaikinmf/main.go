// ChaikinMF example: full compute, streaming continuation, state persistence
// (serialize/deserialize/clone), and both SIMD modes — the Go mirror of
// tulip_rs_ffi/examples/chaikinmf_example.c, with every step verified.
package main
...
func main() {
	...
	info := indicators.Chaikinmf.Info()
	...
	n := 2*int(indicators.Chaikinmf.MinData(options)) + 100
	series := demo.SeriesFor(info.Inputs, n)
	high, low, close, volume := series[0], series[1], series[2], series[3]

	// ---- full compute -----------------------------------------------------
	fmt.Println("\n=== full calculation ===")
	res, st, err := indicators.Chaikinmf.Indicator(high, low, close, volume, options, nil)
	...
	if err == nil {
		...
		fullCmf := append([]float64(nil), tulip.AsFloat64(res.Rows[0])...)
		res.Close()
		st.Close()
		...
		partial := n - 50
		res, st, err := indicators.Chaikinmf.Indicator(high[:partial], low[:partial], close[:partial], volume[:partial], options, nil)
		...
		if err == nil {
			...
			c.Err("batch", err)
			if err == nil {
				tail := fullCmf[len(fullCmf)-len(br.Rows[0]):]
			...
			if err != nil {
				...
				fmt.Printf("  bincode blob: %d bytes (indicator id 0x%08x)\n", len(blob), indicators.ChaikinmfID)
			}
			rs, err := indicators.Chaikinmf.DeserializeState(blob)
	...
	sim, err := indicators.Chaikinmf.SimdByAssets(assets, options, nil)
	c.Err("simd by assets", err)
	if err == nil {
		for i := range sim.Results {
			r, s2, err := indicators.Chaikinmf.Indicator(assets[i][0], assets[i][1], assets[i][2], assets[i][3], options, nil)
	...
	// ---- SIMD by options --------------------------------------------------
	fmt.Println("\n=== SIMD by options (N=4) ===")
	sim2, err := indicators.Chaikinmf.SimdByOptions(high, low, close, volume, [][]float64{{15}, {20}, {25}, {30}}, nil)
	c.Err("simd by options", err)
	if err == nil {
		for i, o := range [][]float64{{15}, {20}, {25}, {30}} {
			r, s2, err := indicators.Chaikinmf.Indicator(high, low, close, volume, o, nil)
