...
// Ad is the namespaced entry point for the Accumulation/Distribution Line.
type ad struct{}

var Ad = ad{}

// AdInputs / AdOptions: series/option arities, from the generated C
// defines (tulip_rs_ffi_counts.h) so they can never drift from the core.
const (
	AdInputs  = int(C.AD_INPUTS)  // high, low, close, volume
	AdOptions = int(C.AD_OPTIONS) // none
)

// AdID is the FFI indicator id for state serialisation: fnv1a32("ad"),
// from the generated tulip_rs_ffi_state_ids.h. Never recompute it here.
var AdID = uint32(C.C_INDICATOR_ID_AD)

// AdState is a live AD streaming-state handle: the state returned by
// Indicator, resumable via Batch, closeable exactly once via Close.
type AdState struct{ *tulip.State }

func adFreeState(h uintptr) { C.ad_state_free(C.u2p(C.uintptr_t(h))) }

func wrapAdState(h uintptr) *AdState {
	return &AdState{tulip.NewState(AdID, h, adFreeState)}
}
...
// MinData returns the minimum number of bars AD needs to produce output.
func (ad) MinData(options []float64) uint64 {
	opts, keep := optionArg(options)
	n := uint64(C.ad_min_data(opts))
	runtime.KeepAlive(keep)
	return n
}

// Indicator computes AD over the given bars.
//
// Outputs: with optionalOutputs empty/nil, Rows is just [ad].
func (ad) Indicator(high, low, close, volume []float64, options []float64, optionalOutputs []bool) (*tulip.Result, *AdState, error) {
	if len(options) != AdOptions {
		return nil, nil, fmt.Errorf("ad: Indicator needs %d option(s), got %d", AdOptions, len(options))
	}
	series := [][]float64{high, low, close, volume}
	arr, release, err := tulip.MarshalSeries(series, len(high))
	if err != nil {
		return nil, nil, fmt.Errorf("ad: %w", err)
	}
	defer release()
	...
	return res, wrapAdState(uintptr(raw.state)), nil
}

// Batch continues the stream with new bars; no need to re-send history.
func (s *AdState) Batch(high, low, close, volume []float64, optionalOutputs []bool) (*tulip.Result, error) {
	if s.Closed() {
...
func (s *AdState) Clone() (*AdState, error) {
	c, err := s.State.Clone()
	if err != nil {
		return nil, err
	}
	return &AdState{c}, nil
}
...
func (ad) DeserializeState(blob []byte) (*AdState, error) {
	st, err := tulip.DeserializeState(AdID, blob, adFreeState)
	if err != nil {
		return nil, fmt.Errorf("ad: %w", err)
	}
	return &AdState{st}, nil
}

// SimdByAssets computes AD for N assets in one CPU pass. Each asset
...
func (ad) SimdByAssets(assets [][AdInputs][]float64, options []float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(options) != AdOptions {
		return nil, fmt.Errorf("ad: SimdByAssets needs %d option(s), got %d", AdOptions, len(options))
	}
