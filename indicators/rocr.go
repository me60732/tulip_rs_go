...
// RocrInputs / RocrOptions: series/option arities, from the generated C
// defines (tulip_rs_ffi_counts.h) so they can never drift from the core.
const (
	RocrInputs  = int(C.ROCR_INPUTS)  // real
	RocrOptions = int(C.ROCR_OPTIONS) // period
)

// RocrID is the FFI indicator id for state serialisation: fnv1a32("rocr"),
// from the generated tulip_rs_ffi_state_ids.h. Never recompute it here.
var RocrID = uint32(C.C_INDICATOR_ID_ROCR)

// RocrState is a live ROCR streaming-state handle: the state returned by
// Indicator, resumable via Batch, closeable exactly once via Close.
type RocrState struct{ *tulip.State }

func rocrFreeState(h uintptr) { C.rocr_state_free(C.u2p(C.uintptr_t(h))) }

func wrapRocrState(h uintptr) *RocrState {
	return &RocrState{tulip.NewState(RocrID, h, rocrFreeState)}
}
...
// MinData returns the minimum number of bars ROCR needs to produce output.
func (rocr) MinData(options []float64) uint64 {
	opts, keep := optionArg(options)
	n := uint64(C.rocr_min_data(opts))
	runtime.KeepAlive(keep)
	return n
}

// Indicator computes ROCR over the given bars.
//
// Outputs: with optionalOutputs empty/nil, Rows is just [rocr]; with one
// flag, Rows is [rocr, highpass]. The returned RocrState is live
// streaming state — closing the Result does NOT free it (and vice versa).
func (rocr) Indicator(real []float64, options []float64, optionalOutputs []bool) (*tulip.Result, *RocrState, error) {
	if len(options) != RocrOptions {
		return nil, nil, fmt.Errorf("rocr: Indicator needs %d option(s) [period], got %d", RocrOptions, len(options))
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	if err != nil {
		return nil, nil, fmt.Errorf("rocr: %w", err)
	}
	defer release()
	...
	return res, wrapRocrState(uintptr(raw.state)), nil
}

// Batch continues the stream with new bars; no need to re-send history.
func (s *RocrState) Batch(real []float64, optionalOutputs []bool) (*tulip.Result, error) {
	if s.Closed() {
	...
	arr, release, err := tulip.MarshalSeries(series, len(real))
	if err != nil {
		return nil, fmt.Errorf("rocr: %w", err)
	}
	defer release()
	optArr, optRelease := tulip.MarshalBools(optionalOutputs)
	defer optRelease()
...
func (s *RocrState) Clone() (*RocrState, error) {
	c, err := s.State.Clone()
	if err != nil {
		return nil, err
	}
	return &RocrState{c}, nil
}
...
func (rocr) DeserializeState(blob []byte) (*RocrState, error) {
	st, err := tulip.DeserializeState(RocrID, blob, rocrFreeState)
	if err != nil {
		return nil, fmt.Errorf("rocr: %w", err)
	}
	return &RocrState{st}, nil
}

// SimdByAssets computes ROCR for N assets in one CPU pass. Each asset
...
func (rocr) SimdByAssets(assets [][RocrInputs][]float64, options []float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(options) != RocrOptions {
		return nil, fmt.Errorf("rocr: SimdByAssets needs %d option(s) [period], got %d", RocrOptions, len(options))
	}
...
// SimdByOptions computes ROCR for N option sets over one shared series of
// bars in one CPU pass. Each lane's resumable state lives in
// SimdResult.States.
func (rocr) SimdByOptions(real []float64, optionSets [][]float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(optionSets) == 0 {
		return nil, fmt.Errorf("rocr: no option sets")
	}
	for i, o := range optionSets {
		if len(o) != RocrOptions {
			return nil, fmt.Errorf("rocr: option set %d has %d values, need %d", i, len(o), RocrOptions)
		}
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	...
	defer release()
	sets := padSets(optionSets, RocrOptions)
