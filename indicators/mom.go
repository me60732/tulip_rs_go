...
// Mom is the namespaced entry point for the Momentum indicator.
type mom struct{}

var Mom = mom{}

// MomInputs / MomOptions: series/option arities, from the generated C
// defines (tulip_rs_ffi_counts.h) so they can never drift from the core.
const (
	MomInputs  = int(C.MOM_INPUTS)  // real
	MomOptions = int(C.MOM_OPTIONS) // period
)

// MomID is the FFI indicator id for state serialisation: fnv1a32("mom"),
// from the generated tulip_rs_ffi_state_ids.h. Never recompute it here.
var MomID = uint32(C.C_INDICATOR_ID_MOM)

// MomState is a live MOM streaming-state handle: the state returned by
// Indicator, resumable via Batch, closeable exactly once via Close.
type MomState struct{ *tulip.State }

func momFreeState(h uintptr) { C.mom_state_free(C.u2p(C.uintptr_t(h))) }

func wrapMomState(h uintptr) *MomState {
	return &MomState{tulip.NewState(MomID, h, momFreeState)}
}
...
// MinData returns the minimum number of bars MOM needs to produce output.
func (mom) MinData(options []float64) uint64 {
	opts, keep := optionArg(options)
	n := uint64(C.mom_min_data(opts))
	runtime.KeepAlive(keep)
	return n
}

// Indicator computes MOM over the given bars.
//
// Outputs: with optionalOutputs empty/nil, Rows is just [mom]. MOM has no
// optional outputs.
func (mom) Indicator(real []float64, options []float64, optionalOutputs []bool) (*tulip.Result, *MomState, error) {
	if len(options) != MomOptions {
		return nil, nil, fmt.Errorf("mom: Indicator needs %d option(s) [period], got %d", MomOptions, len(options))
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	if err != nil {
		return nil, nil, fmt.Errorf("mom: %w", err)
	}
	defer release()
	...
	return res, wrapMomState(uintptr(raw.state)), nil
}

// Batch continues the stream with new bars; no need to re-send history.
func (s *MomState) Batch(real []float64, optionalOutputs []bool) (*tulip.Result, error) {
	if s.Closed() {
	...
	arr, release, err := tulip.MarshalSeries(series, len(real))
	if err != nil {
		return nil, fmt.Errorf("mom: %w", err)
	}
	defer release()
...
func (s *MomState) Clone() (*MomState, error) {
	c, err := s.State.Clone()
	if err != nil {
		return nil, err
	}
	return &MomState{c}, nil
}
...
func (mom) DeserializeState(blob []byte) (*MomState, error) {
	st, err := tulip.DeserializeState(MomID, blob, momFreeState)
	if err != nil {
		return nil, fmt.Errorf("mom: %w", err)
	}
	return &MomState{st}, nil
}

// SimdByAssets computes MOM for N assets in one CPU pass. Each asset
...
func (mom) SimdByAssets(assets [][MomInputs][]float64, options []float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(options) != MomOptions {
		return nil, fmt.Errorf("mom: SimdByAssets needs %d option(s) [period], got %d", MomOptions, len(options))
	}
...
// SimdByOptions computes MOM for N option sets over one shared series of
// bars in one CPU pass. Each lane's resumable state lives in
// SimdResult.States.
func (mom) SimdByOptions(real []float64, optionSets [][]float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(optionSets) == 0 {
		return nil, fmt.Errorf("mom: no option sets")
	}
	for i, o := range optionSets {
		if len(o) != MomOptions {
			return nil, fmt.Errorf("mom: option set %d has %d values, need %d", i, len(o), MomOptions)
		}
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	...
	defer release()
	sets := padSets(optionSets, MomOptions)
