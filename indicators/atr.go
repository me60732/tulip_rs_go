...
// Atr is the namespaced entry point for the Average True Range.
type atr struct{}

var Atr = atr{}

// AtrInputs / AtrOptions: series/option arities, from the generated C
// defines (tulip_rs_ffi_counts.h) so they can never drift from the core.
const (
	AtrInputs  = int(C.ATR_INPUTS)  // high, low, close
	AtrOptions = int(C.ATR_OPTIONS) // period
)

// AtrID is the FFI indicator id for state serialisation: fnv1a32("atr"),
// from the generated tulip_rs_ffi_state_ids.h. Never recompute it here.
var AtrID = uint32(C.C_INDICATOR_ID_ATR)

// AtrState is a live ATR streaming-state handle: the state returned by
// Indicator, resumable via Batch, closeable exactly once via Close.
type AtrState struct{ *tulip.State }

func atrFreeState(h uintptr) { C.atr_state_free(C.u2p(C.uintptr_t(h))) }

func wrapAtrState(h uintptr) *AtrState {
	return &AtrState{tulip.NewState(AtrID, h, atrFreeState)}
}
...
// MinData returns the minimum number of bars ATR needs to produce output.
func (atr) MinData(options []float64) uint64 {
	opts, keep := optionArg(options)
	n := uint64(C.atr_min_data(opts))
	runtime.KeepAlive(keep)
	return n
}

// Indicator computes ATR over the given bars.
//
// Outputs: Rows is just [atr].
func (atr) Indicator(high, low, close []float64, options []float64, optionalOutputs []bool) (*tulip.Result, *AtrState, error) {
	if len(options) != AtrOptions {
		return nil, nil, fmt.Errorf("atr: Indicator needs %d option(s) [period], got %d", AtrOptions, len(options))
	}
	series := [][]float64{high, low, close}
	arr, release, err := tulip.MarshalSeries(series, len(high))
	if err != nil {
		return nil, nil, fmt.Errorf("atr: %w", err)
	}
	defer release()
	...
	return res, wrapAtrState(uintptr(raw.state)), nil
}

// Batch continues the stream with new bars; no need to re-send history.
func (s *AtrState) Batch(high, low, close []float64, optionalOutputs []bool) (*tulip.Result, error) {
	if s.Closed() {
...
func (s *AtrState) Clone() (*AtrState, error) {
	c, err := s.State.Clone()
	if err != nil {
		return nil, err
	}
	return &AtrState{c}, nil
}
...
func (atr) DeserializeState(blob []byte) (*AtrState, error) {
	st, err := tulip.DeserializeState(AtrID, blob, atrFreeState)
	if err != nil {
		return nil, fmt.Errorf("atr: %w", err)
	}
	return &AtrState{st}, nil
}

// SimdByAssets computes ATR for N assets in one CPU pass. Each asset
// carries the same series (high, low, close) of equal length; all lanes share one options set.
func (atr) SimdByAssets(assets [][AtrInputs][]float64, options []float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(options) != AtrOptions {
		return nil, fmt.Errorf("atr: SimdByAssets needs %d option(s) [period], got %d", AtrOptions, len(options))
	}
...
// SimdByOptions computes ATR for N option sets over one shared series of
// bars in one CPU pass. Each lane's resumable state lives in
// SimdResult.States.
func (atr) SimdByOptions(high, low, close []float64, optionSets [][]float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(optionSets) == 0 {
		return nil, fmt.Errorf("atr: no option sets")
	}
	for i, o := range optionSets {
		if len(o) != AtrOptions {
			return nil, fmt.Errorf("atr: option set %d has %d values, need %d", i, len(o), AtrOptions)
		}
	}
	series := [][]float64{high, low, close}
	arr, release, err := tulip.MarshalSeries(series, len(high))
	...
	defer release()
	sets := padSets(optionSets, AtrOptions)
