...
// Apo is the namespaced entry point for the Average Price Oscillator.
type apo struct{}

var Apo = apo{}

// ApoInputs / ApoOptions: series/option arities, from the generated C
// defines (tulip_rs_ffi_counts.h) so they can never drift from the core.
const (
	ApoInputs  = int(C.APO_INPUTS)  // real
	ApoOptions = int(C.APO_OPTIONS) // short_period, long_period
)

// ApoID is the FFI indicator id for state serialisation: fnv1a32("apo"),
// from the generated tulip_rs_ffi_state_ids.h. Never recompute it here.
var ApoID = uint32(C.C_INDICATOR_ID_APO)

// ApoState is a live APO streaming-state handle: the state returned by
// Indicator, resumable via Batch, closeable exactly once via Close.
type ApoState struct{ *tulip.State }

func apoFreeState(h uintptr) { C.apo_state_free(C.u2p(C.uintptr_t(h))) }

func wrapApoState(h uintptr) *ApoState {
	return &ApoState{tulip.NewState(ApoID, h, apoFreeState)}
}
...
// MinData returns the minimum number of bars APO needs to produce output.
func (apo) MinData(options []float64) uint64 {
	opts, keep := optionArg(options)
	n := uint64(C.apo_min_data(opts))
	runtime.KeepAlive(keep)
	return n
}

// Indicator computes APO over the given bars.
//
// Outputs: Rows is just [apo].
func (apo) Indicator(real []float64, options []float64, optionalOutputs []bool) (*tulip.Result, *ApoState, error) {
	if len(options) != ApoOptions {
		return nil, nil, fmt.Errorf("apo: Indicator needs %d option(s) [short_period, long_period], got %d", ApoOptions, len(options))
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	if err != nil {
		return nil, nil, fmt.Errorf("apo: %w", err)
	}
	defer release()
	...
	return res, wrapApoState(uintptr(raw.state)), nil
}

// Batch continues the stream with new bars; no need to re-send history.
func (s *ApoState) Batch(real []float64, optionalOutputs []bool) (*tulip.Result, error) {
	if s.Closed() {
	...
	arr, release, err := tulip.MarshalSeries(series, len(real))
	if err != nil {
		return nil, fmt.Errorf("apo: %w", err)
	}
	defer release()
	optArr, optRelease := tulip.MarshalBools(optionalOutputs)
	defer optRelease()
...
func (s *ApoState) Clone() (*ApoState, error) {
	c, err := s.State.Clone()
	if err != nil {
		return nil, err
	}
	return &ApoState{c}, nil
}
...
func (apo) DeserializeState(blob []byte) (*ApoState, error) {
	st, err := tulip.DeserializeState(ApoID, blob, apoFreeState)
	if err != nil {
		return nil, fmt.Errorf("apo: %w", err)
	}
	return &ApoState{st}, nil
}

// SimdByAssets computes APO for N assets in one CPU pass. Each asset
// carries the same series (real) of equal length; all lanes share one options set.
func (apo) SimdByAssets(assets [][ApoInputs][]float64, options []float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(options) != ApoOptions {
		return nil, fmt.Errorf("apo: SimdByAssets needs %d option(s) [short_period, long_period], got %d", ApoOptions, len(options))
	}
...
// SimdByOptions computes APO for N option sets over one shared series of
// bars in one CPU pass. Each lane's resumable state lives in
// SimdResult.States.
func (apo) SimdByOptions(real []float64, optionSets [][]float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(optionSets) == 0 {
		return nil, fmt.Errorf("apo: no option sets")
	}
	for i, o := range optionSets {
		if len(o) != ApoOptions {
			return nil, fmt.Errorf("apo: option set %d has %d values, need %d", i, len(o), ApoOptions)
		}
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	...
	defer release()
	sets := padSets(optionSets, ApoOptions)
