...
// Tsf is the namespaced entry point for the Time Series Forecast.
type tsf struct{}

var Tsf = tsf{}

// TsfInputs / TsfOptions: series/option arities, from the generated C
// defines (tulip_rs_ffi_counts.h) so they can never drift from the core.
const (
	TsfInputs  = int(C.TSF_INPUTS)  // real
	TsfOptions = int(C.TSF_OPTIONS) // period
)

// TsfID is the FFI indicator id for state serialisation: fnv1a32("tsf"),
// from the generated tulip_rs_ffi_state_ids.h. Never recompute it here.
var TsfID = uint32(C.C_INDICATOR_ID_TSF)

// TsfState is a live TSF streaming-state handle: the state returned by
// Indicator, resumable via Batch, closeable exactly once via Close.
type TsfState struct{ *tulip.State }

func tsfFreeState(h uintptr) { C.tsf_state_free(C.u2p(C.uintptr_t(h))) }

func wrapTsfState(h uintptr) *TsfState {
	return &TsfState{tulip.NewState(TsfID, h, tsfFreeState)}
}
...
// MinData returns the minimum number of bars TSF needs to produce output.
func (tsf) MinData(options []float64) uint64 {
	opts, keep := optionArg(options)
	n := uint64(C.tsf_min_data(opts))
	runtime.KeepAlive(keep)
	return n
}

// Indicator computes TSF over the given bars.
//
// Outputs: with optionalOutputs empty/nil, Rows is just [tsf]; with three
// flags, Rows is [tsf, linreg, linregslope, linregintercept]. The returned
// TsfState is live streaming state — closing the Result does NOT free it (and vice versa).
func (tsf) Indicator(real []float64, options []float64, optionalOutputs []bool) (*tulip.Result, *TsfState, error) {
	if len(options) != TsfOptions {
		return nil, nil, fmt.Errorf("tsf: Indicator needs %d option(s) [period], got %d", TsfOptions, len(options))
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	if err != nil {
		return nil, nil, fmt.Errorf("tsf: %w", err)
	}
	defer release()
	...
	return res, wrapTsfState(uintptr(raw.state)), nil
}

// Batch continues the stream with new bars; no need to re-send history.
func (s *TsfState) Batch(real []float64, optionalOutputs []bool) (*tulip.Result, error) {
	if s.Closed() {
...
func (s *TsfState) Clone() (*TsfState, error) {
	c, err := s.State.Clone()
	if err != nil {
		return nil, err
	}
	return &TsfState{c}, nil
}
...
func (tsf) DeserializeState(blob []byte) (*TsfState, error) {
	st, err := tulip.DeserializeState(TsfID, blob, tsfFreeState)
	if err != nil {
		return nil, fmt.Errorf("tsf: %w", err)
	}
	return &TsfState{st}, nil
}

// SimdByAssets computes TSF for N assets in one CPU pass. Each asset
...
func (tsf) SimdByAssets(assets [][TsfInputs][]float64, options []float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(options) != TsfOptions {
		return nil, fmt.Errorf("tsf: SimdByAssets needs %d option(s) [period], got %d", TsfOptions, len(options))
	}
...
// SimdByOptions computes TSF for N option sets over one shared series of
// bars in one CPU pass. Each lane's resumable state lives in
// SimdResult.States.
func (tsf) SimdByOptions(real []float64, optionSets [][]float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(optionSets) == 0 {
		return nil, fmt.Errorf("tsf: no option sets")
	}
	for i, o := range optionSets {
		if len(o) != TsfOptions {
			return nil, fmt.Errorf("tsf: option set %d has %d values, need %d", i, len(o), TsfOptions)
		}
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	...
	defer release()
	sets := padSets(optionSets, TsfOptions)
