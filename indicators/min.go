package indicators

/*
#cgo CFLAGS: -I${SRCDIR}/../../tulip_rs_ffi/include
#include <stdbool.h>
#include <stdint.h>
// cgo prologue defines a `CBytes` helper; rename the FFI typedef to avoid
// the clash (must match the rename in every cgo file of this package and in
// the tulip package's preamble).
#define CBytes TulipBytes
#include "tulip_rs_ffi.h"

static inline void *u2p(uintptr_t p) { return (void *)p; }
*/
import "C"

import (
	"fmt"
	"runtime"
	"unsafe"

	"tulip_rs_go/tulip"
)

// Min is the namespaced entry point for the Minimum indicator.
type min struct{}

var Min = min{}

// MinInputs / MinOptions: series/option arities, from the generated C
// defines (tulip_rs_ffi_counts.h) so they can never drift from the core.
const (
	MinInputs  = int(C.MIN_INPUTS)  // real
	MinOptions = int(C.MIN_OPTIONS) // period
)

// MinID is the FFI indicator id for state serialisation: fnv1a32("min"),
// from the generated tulip_rs_ffi_state_ids.h. Never recompute it here.
var MinID = uint32(C.C_INDICATOR_ID_MIN)

// MinState is a live MIN streaming-state handle: the state returned by
// Indicator, resumable via Batch, closeable exactly once via Close.
type MinState struct{ *tulip.State }

func minFreeState(h uintptr) { C.min_state_free(C.u2p(C.uintptr_t(h))) }

func wrapMinState(h uintptr) *MinState {
	return &MinState{tulip.NewState(MinID, h, minFreeState)}
}

// Info returns the indicator's metadata as pure Go values. The underlying
// C strings are process-lifetime; they are copied and never freed.
func (min) Info() tulip.Info {
	ci := C.min_info()
	return tulip.NewInfo(
		int32(ci.indicator_type),
		C.GoString(ci.name),
		C.GoString(ci.full_name),
		tulip.CopyStringArray(uintptr(unsafe.Pointer(ci.inputs.ptr)), int(ci.inputs.len)),
		tulip.CopyStringArray(uintptr(unsafe.Pointer(ci.options.ptr)), int(ci.options.len)),
		tulip.CopyStringArray(uintptr(unsafe.Pointer(ci.outputs.ptr)), int(ci.outputs.len)),
		tulip.CopyStringArray(uintptr(unsafe.Pointer(ci.optional_outputs.ptr)), int(ci.optional_outputs.len)),
	)
}

// MinData returns the minimum number of bars MIN needs to produce output.
func (min) MinData(options []float64) uint64 {
	opts, keep := optionArg(options)
	n := uint64(C.min_min_data(opts))
	runtime.KeepAlive(keep)
	return n
}

// Indicator computes MIN over the given bars.
//
// Outputs: with optionalOutputs empty/nil, Rows is just [min]; there are no
// optional outputs for this indicator.
func (min) Indicator(real []float64, options []float64, optionalOutputs []bool) (*tulip.Result, *MinState, error) {
	if len(options) != MinOptions {
		return nil, nil, fmt.Errorf("min: Indicator needs %d option(s) [period], got %d", MinOptions, len(options))
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	if err != nil {
		return nil, nil, fmt.Errorf("min: %w", err)
	}
	defer release()
	opts, optsKeep := optionArg(options)

	raw := C.min_indicator(
		(**C.double)(arr),
		C.size_t(len(real)),
		opts,
		nil,
		0,
	)
	res, err := tulip.NewResult(
		int32(raw.error),
		unsafe.Pointer(raw.outputs),
		unsafe.Pointer(raw.output_lens),
		int(raw.num_outputs),
		uintptr(raw.state),
	)
	runtime.KeepAlive(series)
	runtime.KeepAlive(optsKeep)
	if err != nil {
		return nil, nil, fmt.Errorf("min: %w", err)
	}
	return res, wrapMinState(uintptr(raw.state)), nil
}

// Batch continues the stream with new bars; no need to re-send history.
func (s *MinState) Batch(real []float64) (*tulip.Result, error) {
	if s.Closed() {
		return nil, tulip.ErrClosed
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	if err != nil {
		return nil, fmt.Errorf("min: %w", err)
	}
	defer release()

	raw := C.min_batch(
		s.Handle(),
		(**C.double)(arr),
		C.size_t(len(real)),
		nil,
		0,
	)
	res, err := tulip.NewBatchResult(
		int32(raw.error),
		unsafe.Pointer(raw.outputs),
		unsafe.Pointer(raw.output_lens),
		int(raw.num_outputs),
	)
	runtime.KeepAlive(series)
	if err != nil {
		return nil, fmt.Errorf("min: %w", err)
	}
	return res, nil
}

// Clone returns an independent deep copy of the state (no serde involved).
func (s *MinState) Clone() (*MinState, error) {
	c, err := s.State.Clone()
	if err != nil {
		return nil, err
	}
	return &MinState{c}, nil
}

// DeserializeState restores a streaming state from a TRFS blob produced by
// State.Serialize (from this or any other tulip_rs_ffi binding).
func (min) DeserializeState(blob []byte) (*MinState, error) {
	st, err := tulip.DeserializeState(MinID, blob, minFreeState)
	if err != nil {
		return nil, fmt.Errorf("min: %w", err)
	}
	return &MinState{st}, nil
}

// SimdByAssets computes MIN for N assets in one CPU pass. Each asset
// carries the same series (real) of equal length; all lanes share one options set.
func (min) SimdByAssets(assets [][MinInputs][]float64, options []float64) (*tulip.SimdResult, error) {
	if len(options) != MinOptions {
		return nil, fmt.Errorf("min: SimdByAssets needs %d option(s) [period], got %d", MinOptions, len(options))
	}
	groups := make([][][]float64, len(assets))
	for i, a := range assets {
		groups[i] = [][]float64{a[0]}
	}
	outer, release, err := tulip.MarshalSimdInputs(groups, len(assets[0][0]))
	if err != nil {
		return nil, fmt.Errorf("min: %w", err)
	}
	defer release()
	opts, optsKeep := optionArg(options)

	raw := C.min_simd_by_assets(
		(***C.double)(outer),
		C.size_t(len(assets)),
		C.size_t(len(assets[0][0])),
		opts,
		nil,
		0,
	)
	res, err := tulip.NewSimdResult(
		int32(raw.error),
		unsafe.Pointer(raw.outputs),
		unsafe.Pointer(raw.output_lens),
		unsafe.Pointer(raw.states),
		int(raw.num_results),
		int(raw.num_outputs),
		MinID, minFreeState,
	)
	runtime.KeepAlive(assets)
	runtime.KeepAlive(optsKeep)
	if err != nil {
		return nil, fmt.Errorf("min: %w", err)
	}
	return res, nil
}

// SimdByOptions computes MIN for N option sets over one shared series of
// bars in one CPU pass. Each lane's resumable state lives in
// SimdResult.States.
func (min) SimdByOptions(real []float64, optionSets [][]float64) (*tulip.SimdResult, error) {
	if len(optionSets) == 0 {
		return nil, fmt.Errorf("min: no option sets")
	}
	for i, o := range optionSets {
		if len(o) != MinOptions {
			return nil, fmt.Errorf("min: option set %d has %d values, need %d", i, len(o), MinOptions)
		}
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	if err != nil {
		return nil, fmt.Errorf("min: %w", err)
	}
	defer release()
	sets := padSets(optionSets, MinOptions)
	optsArr, optsRelease, err := tulip.MarshalSeries(sets, len(sets[0]))
	if err != nil {
		return nil, fmt.Errorf("min: %w", err)
	}
	defer optsRelease()

	raw := C.min_simd_by_options(
		(**C.double)(arr),
		C.size_t(len(real)),
		(**C.double)(optsArr),
		C.size_t(len(optionSets)),
		nil,
		0,
	)
	res, err := tulip.NewSimdResult(
		int32(raw.error),
		unsafe.Pointer(raw.outputs),
		unsafe.Pointer(raw.output_lens),
		unsafe.Pointer(raw.states),
		int(raw.num_results),
		int(raw.num_outputs),
		MinID, minFreeState,
	)
	runtime.KeepAlive(series)
	runtime.KeepAlive(sets)
	if err != nil {
		return nil, fmt.Errorf("min: %w", err)
	}
	return res, nil
}
