...
// Hma is the namespaced entry point for the Hull Moving Average.
type hma struct{}

var Hma = hma{}

// HmaInputs / HmaOptions: series/option arities, from the generated C
// defines (tulip_rs_ffi_counts.h) so they can never drift from the core.
const (
	HmaInputs  = int(C.HMA_INPUTS)  // real
	HmaOptions = int(C.HMA_OPTIONS) // period
)

// HmaID is the FFI indicator id for state serialisation: fnv1a32("hma"),
// from the generated tulip_rs_ffi_state_ids.h. Never recompute it here.
var HmaID = uint32(C.C_INDICATOR_ID_HMA)

// HmaState is a live HMA streaming-state handle: the state returned by
// Indicator, resumable via Batch, closeable exactly once via Close.
type HmaState struct{ *tulip.State }

func hmaFreeState(h uintptr) { C.hma_state_free(C.u2p(C.uintptr_t(h))) }

func wrapHmaState(h uintptr) *HmaState {
	return &HmaState{tulip.NewState(HmaID, h, hmaFreeState)}
}
...
// MinData returns the minimum number of bars HMA needs to produce output.
func (hma) MinData(options []float64) uint64 {
	opts, keep := optionArg(options)
	n := uint64(C.hma_min_data(opts))
	runtime.KeepAlive(keep)
	return n
}

// Indicator computes HMA over the given bars.
//
// Outputs: with optionalOutputs empty/nil, Rows is just [hma]. The returned HmaState
// is live streaming state — closing the Result does NOT free it (and vice versa).
func (hma) Indicator(real []float64, options []float64, optionalOutputs []bool) (*tulip.Result, *HmaState, error) {
	if len(options) != HmaOptions {
		return nil, nil, fmt.Errorf("hma: Indicator needs %d option(s) [period], got %d", HmaOptions, len(options))
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	if err != nil {
		return nil, nil, fmt.Errorf("hma: %w", err)
	}
	defer release()
	...
	return res, wrapHmaState(uintptr(raw.state)), nil
}

// Batch continues the stream with new bars; no need to re-send history.
func (s *HmaState) Batch(real []float64, optionalOutputs []bool) (*tulip.Result, error) {
	if s.Closed() {
...
func (s *HmaState) Clone() (*HmaState, error) {
	c, err := s.State.Clone()
	if err != nil {
		return nil, err
	}
	return &HmaState{c}, nil
}
...
func (hma) DeserializeState(blob []byte) (*HmaState, error) {
	st, err := tulip.DeserializeState(HmaID, blob, hmaFreeState)
	if err != nil {
		return nil, fmt.Errorf("hma: %w", err)
	}
	return &HmaState{st}, nil
}

// SimdByAssets computes HMA for N assets in one CPU pass. Each asset
// carries the same series (real) of equal length; all lanes share one options set.
func (hma) SimdByAssets(assets [][HmaInputs][]float64, options []float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(options) != HmaOptions {
		return nil, fmt.Errorf("hma: SimdByAssets needs %d option(s) [period], got %d", HmaOptions, len(options))
	}
...
// SimdByOptions computes HMA for N option sets over one shared series of
// bars in one CPU pass. Each lane's resumable state lives in
// SimdResult.States.
func (hma) SimdByOptions(real []float64, optionSets [][]float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(optionSets) == 0 {
		return nil, fmt.Errorf("hma: no option sets")
	}
	for i, o := range optionSets {
		if len(o) != HmaOptions {
			return nil, fmt.Errorf("hma: option set %d has %d values, need %d", i, len(o), HmaOptions)
		}
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	...
	defer release()
	sets := padSets(optionSets, HmaOptions)
