...
// Ppo is the namespaced entry point for the Percentage Price Oscillator.
type ppo struct{}

var Ppo = ppo{}

// PpoInputs / PpoOptions: series/option arities, from the generated C
// defines (tulip_rs_ffi_counts.h) so they can never drift from the core.
const (
	PpoInputs  = int(C.PPO_INPUTS)  // real
	PpoOptions = int(C.PPO_OPTIONS) // fast_period, slow_period
)

// PpoID is the FFI indicator id for state serialisation: fnv1a32("ppo"),
// from the generated tulip_rs_ffi_state_ids.h. Never recompute it here.
var PpoID = uint32(C.C_INDICATOR_ID_PPO)

// PpoState is a live PPO streaming-state handle: the state returned by
// Indicator, resumable via Batch, closeable exactly once via Close.
type PpoState struct{ *tulip.State }

func ppoFreeState(h uintptr) { C.ppo_state_free(C.u2p(C.uintptr_t(h))) }

func wrapPpoState(h uintptr) *PpoState {
	return &PpoState{tulip.NewState(PpoID, h, ppoFreeState)}
}
...
// MinData returns the minimum number of bars PPO needs to produce output.
func (ppo) MinData(options []float64) uint64 {
	opts, keep := optionArg(options)
	n := uint64(C.ppo_min_data(opts))
	runtime.KeepAlive(keep)
	return n
}

// Indicator computes PPO over the given bars.
//
// Outputs: with optionalOutputs empty/nil, Rows is just [ppo]; with two
// flags, Rows is [ppo, short_ema, long_ema].
func (ppo) Indicator(real []float64, options []float64, optionalOutputs []bool) (*tulip.Result, *PpoState, error) {
	if len(options) != PpoOptions {
		return nil, nil, fmt.Errorf("ppo: Indicator needs %d option(s) [fast_period, slow_period], got %d", PpoOptions, len(options))
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	if err != nil {
		return nil, nil, fmt.Errorf("ppo: %w", err)
	}
	defer release()
	...
	return res, wrapPpoState(uintptr(raw.state)), nil
}

// Batch continues the stream with new bars; no need to re-send history.
func (s *PpoState) Batch(real []float64, optionalOutputs []bool) (*tulip.Result, error) {
	if s.Closed() {
	...
	arr, release, err := tulip.MarshalSeries(series, len(real))
	if err != nil {
		return nil, fmt.Errorf("ppo: %w", err)
	}
	defer release()
	optArr, optRelease := tulip.MarshalBools(optionalOutputs)
	defer optRelease()
...
func (s *PpoState) Clone() (*PpoState, error) {
	c, err := s.State.Clone()
	if err != nil {
		return nil, err
	}
	return &PpoState{c}, nil
}
...
func (ppo) DeserializeState(blob []byte) (*PpoState, error) {
	st, err := tulip.DeserializeState(PpoID, blob, ppoFreeState)
	if err != nil {
		return nil, fmt.Errorf("ppo: %w", err)
	}
	return &PpoState{st}, nil
}

// SimdByAssets computes PPO for N assets in one CPU pass. Each asset
...
func (ppo) SimdByAssets(assets [][PpoInputs][]float64, options []float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(options) != PpoOptions {
		return nil, fmt.Errorf("ppo: SimdByAssets needs %d option(s) [fast_period, slow_period], got %d", PpoOptions, len(options))
	}
...
// SimdByOptions computes PPO for N option sets over one shared series of
// bars in one CPU pass. Each lane's resumable state lives in
// SimdResult.States.
func (ppo) SimdByOptions(real []float64, optionSets [][]float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(optionSets) == 0 {
		return nil, fmt.Errorf("ppo: no option sets")
	}
	for i, o := range optionSets {
		if len(o) != PpoOptions {
			return nil, fmt.Errorf("ppo: option set %d has %d values, need %d", i, len(o), PpoOptions)
		}
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	...
	defer release()
	sets := padSets(optionSets, PpoOptions)
