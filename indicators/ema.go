...
// Ema is the namespaced entry point for the Exponential Moving Average.
type ema struct{}

var Ema = ema{}

// EmaInputs / EmaOptions: series/option arities, from the generated C
// defines (tulip_rs_ffi_counts.h) so they can never drift from the core.
const (
	EmaInputs  = int(C.EMA_INPUTS)  // real
	EmaOptions = int(C.EMA_OPTIONS) // period
)

// EmaID is the FFI indicator id for state serialisation: fnv1a32("ema"),
// from the generated tulip_rs_ffi_state_ids.h. Never recompute it here.
var EmaID = uint32(C.C_INDICATOR_ID_EMA)

// EmaState is a live EMA streaming-state handle: the state returned by
// Indicator, resumable via Batch, closeable exactly once via Close.
type EmaState struct{ *tulip.State }

func emaFreeState(h uintptr) { C.ema_state_free(C.u2p(C.uintptr_t(h))) }

func wrapEmaState(h uintptr) *EmaState {
	return &EmaState{tulip.NewState(EmaID, h, emaFreeState)}
}
...
// MinData returns the minimum number of bars EMA needs to produce output.
func (ema) MinData(options []float64) uint64 {
	opts, keep := optionArg(options)
	n := uint64(C.ema_min_data(opts))
	runtime.KeepAlive(keep)
	return n
}

// Indicator computes EMA over the given bars.
//
// Outputs: with optionalOutputs empty/nil, Rows is just [ema].
func (ema) Indicator(real []float64, options []float64, optionalOutputs []bool) (*tulip.Result, *EmaState, error) {
	if len(options) != EmaOptions {
		return nil, nil, fmt.Errorf("ema: Indicator needs %d option(s) [period], got %d", EmaOptions, len(options))
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	if err != nil {
		return nil, nil, fmt.Errorf("ema: %w", err)
	}
	defer release()
	...
	return res, wrapEmaState(uintptr(raw.state)), nil
}

// Batch continues the stream with new bars; no need to re-send history.
func (s *EmaState) Batch(real []float64, optionalOutputs []bool) (*tulip.Result, error) {
	if s.Closed() {
...
func (s *EmaState) Clone() (*EmaState, error) {
	c, err := s.State.Clone()
	if err != nil {
		return nil, err
	}
	return &EmaState{c}, nil
}
...
func (ema) DeserializeState(blob []byte) (*EmaState, error) {
	st, err := tulip.DeserializeState(EmaID, blob, emaFreeState)
	if err != nil {
		return nil, fmt.Errorf("ema: %w", err)
	}
	return &EmaState{st}, nil
}

// SimdByAssets computes EMA for N assets in one CPU pass. Each asset
...
func (ema) SimdByAssets(assets [][EmaInputs][]float64, options []float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(options) != EmaOptions {
		return nil, fmt.Errorf("ema: SimdByAssets needs %d option(s) [period], got %d", EmaOptions, len(options))
	}
...
// SimdByOptions computes EMA for N option sets over one shared series of
// bars in one CPU pass. Each lane's resumable state lives in
// SimdResult.States.
func (ema) SimdByOptions(real []float64, optionSets [][]float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(optionSets) == 0 {
		return nil, fmt.Errorf("ema: no option sets")
	}
	for i, o := range optionSets {
		if len(o) != EmaOptions {
			return nil, fmt.Errorf("ema: option set %d has %d values, need %d", i, len(o), EmaOptions)
		}
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	...
	defer release()
	sets := padSets(optionSets, EmaOptions)
