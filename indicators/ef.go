...
// Ef is the namespaced entry point for the Efficiency Ratio.
type ef struct{}

var Ef = ef{}

// EfInputs / EfOptions: series/option arities, from the generated C
// defines (tulip_rs_ffi_counts.h) so they can never drift from the core.
const (
	EfInputs  = int(C.EF_INPUTS)  // real
	EfOptions = int(C.EF_OPTIONS) // period
)

// EfID is the FFI indicator id for state serialisation: fnv1a32("ef"),
// from the generated tulip_rs_ffi_state_ids.h. Never recompute it here.
var EfID = uint32(C.C_INDICATOR_ID_EF)

// EfState is a live EF streaming-state handle: the state returned by
// Indicator, resumable via Batch, closeable exactly once via Close.
type EfState struct{ *tulip.State }

func efFreeState(h uintptr) { C.ef_state_free(C.u2p(C.uintptr_t(h))) }

func wrapEfState(h uintptr) *EfState {
	return &EfState{tulip.NewState(EfID, h, efFreeState)}
}
...
// MinData returns the minimum number of bars EF needs to produce output.
func (ef) MinData(options []float64) uint64 {
	opts, keep := optionArg(options)
	n := uint64(C.ef_min_data(opts))
	runtime.KeepAlive(keep)
	return n
}

// Indicator computes EF over the given bars.
//
// Outputs: ef. The returned EfState is live streaming state — closing the
// Result does NOT free it (and vice versa).
func (ef) Indicator(real []float64, options []float64, optionalOutputs []bool) (*tulip.Result, *EfState, error) {
	if len(options) != EfOptions {
		return nil, nil, fmt.Errorf("ef: Indicator needs %d option(s) [period], got %d", EfOptions, len(options))
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	if err != nil {
		return nil, nil, fmt.Errorf("ef: %w", err)
	}
	defer release()
	...
	return res, wrapEfState(uintptr(raw.state)), nil
}

// Batch continues the stream with new bars; no need to re-send history.
func (s *EfState) Batch(real []float64, optionalOutputs []bool) (*tulip.Result, error) {
	if s.Closed() {
...
func (s *EfState) Clone() (*EfState, error) {
	c, err := s.State.Clone()
	if err != nil {
		return nil, err
	}
	return &EfState{c}, nil
}
...
func (ef) DeserializeState(blob []byte) (*EfState, error) {
	st, err := tulip.DeserializeState(EfID, blob, efFreeState)
	if err != nil {
		return nil, fmt.Errorf("ef: %w", err)
	}
	return &EfState{st}, nil
}

// SimdByAssets computes EF for N assets in one CPU pass. Each asset
...
func (ef) SimdByAssets(assets [][EfInputs][]float64, options []float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(options) != EfOptions {
		return nil, fmt.Errorf("ef: SimdByAssets needs %d option(s) [period], got %d", EfOptions, len(options))
	}
...
// SimdByOptions computes EF for N option sets over one shared series of
// bars in one CPU pass. Each lane's resumable state lives in
// SimdResult.States.
func (ef) SimdByOptions(real []float64, optionSets [][]float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(optionSets) == 0 {
		return nil, fmt.Errorf("ef: no option sets")
	}
	for i, o := range optionSets {
		if len(o) != EfOptions {
			return nil, fmt.Errorf("ef: option set %d has %d values, need %d", i, len(o), EfOptions)
		}
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	...
	defer release()
	sets := padSets(optionSets, EfOptions)
