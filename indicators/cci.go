...
// Cci is the namespaced entry point for the Commodity Channel Index.
type cci struct{}

var Cci = cci{}

// CciInputs / CciOptions: series/option arities, from the generated C
// defines (tulip_rs_ffi_counts.h) so they can never drift from the core.
const (
	CciInputs  = int(C.CCI_INPUTS)  // high, low, close
	CciOptions = int(C.CCI_OPTIONS) // period
)

// CciID is the FFI indicator id for state serialisation: fnv1a32("cci"),
// from the generated tulip_rs_ffi_state_ids.h. Never recompute it here.
var CciID = uint32(C.C_INDICATOR_ID_CCI)

// CciState is a live CCI streaming-state handle: the state returned by
// Indicator, resumable via Batch, closeable exactly once via Close.
type CciState struct{ *tulip.State }

func cciFreeState(h uintptr) { C.cci_state_free(C.u2p(C.uintptr_t(h))) }

func wrapCciState(h uintptr) *CciState {
	return &CciState{tulip.NewState(CciID, h, cciFreeState)}
}
...
// MinData returns the minimum number of bars CCI needs to produce output.
func (cci) MinData(options []float64) uint64 {
	opts, keep := optionArg(options)
	n := uint64(C.cci_min_data(opts))
	runtime.KeepAlive(keep)
	return n
}

// Indicator computes CCI over the given bars.
//
// Outputs: with optionalOutputs empty/nil, Rows is [cci]; with flags,
// Rows adds [sma, md, typprice].
func (cci) Indicator(high, low, close []float64, options []float64, optionalOutputs []bool) (*tulip.Result, *CciState, error) {
	if len(options) != CciOptions {
		return nil, nil, fmt.Errorf("cci: Indicator needs %d option(s) [period], got %d", CciOptions, len(options))
	}
	series := [][]float64{high, low, close}
	arr, release, err := tulip.MarshalSeries(series, len(high))
	if err != nil {
		return nil, nil, fmt.Errorf("cci: %w", err)
	}
	defer release()
	...
	return res, wrapCciState(uintptr(raw.state)), nil
}

// Batch continues the stream with new bars; no need to re-send history.
func (s *CciState) Batch(high, low, close []float64, optionalOutputs []bool) (*tulip.Result, error) {
	if s.Closed() {
...
func (s *CciState) Clone() (*CciState, error) {
	c, err := s.State.Clone()
	if err != nil {
		return nil, err
	}
	return &CciState{c}, nil
}
...
func (cci) DeserializeState(blob []byte) (*CciState, error) {
	st, err := tulip.DeserializeState(CciID, blob, cciFreeState)
	if err != nil {
		return nil, fmt.Errorf("cci: %w", err)
	}
	return &CciState{st}, nil
}

// SimdByAssets computes CCI for N assets in one CPU pass. Each asset
// carries the same series (high, low, close) of equal length; all lanes
// share one options set.
func (cci) SimdByAssets(assets [][CciInputs][]float64, options []float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(options) != CciOptions {
		return nil, fmt.Errorf("cci: SimdByAssets needs %d option(s) [period], got %d", CciOptions, len(options))
	}
...
// SimdByOptions computes CCI for N option sets over one shared series of
// bars in one CPU pass. Each lane's resumable state lives in SimdResult.States.
func (cci) SimdByOptions(high, low, close []float64, optionSets [][]float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(optionSets) == 0 {
		return nil, fmt.Errorf("cci: no option sets")
	}
	for i, o := range optionSets {
		if len(o) != CciOptions {
			return nil, fmt.Errorf("cci: option set %d has %d values, need %d", i, len(o), CciOptions)
		}
	}
	series := [][]float64{high, low, close}
	arr, release, err := tulip.MarshalSeries(series, len(high))
	...
	defer release()
	sets := padSets(optionSets, CciOptions)
