...
// Cmo is the namespaced entry point for the Chande Momentum Oscillator.
type cmo struct{}

var Cmo = cmo{}

// CmoInputs / CmoOptions: series/option arities, from the generated C
// defines (tulip_rs_ffi_counts.h) so they can never drift from the core.
const (
	CmoInputs  = int(C.CMO_INPUTS)  // real
	CmoOptions = int(C.CMO_OPTIONS) // period
)

// CmoID is the FFI indicator id for state serialisation: fnv1a32("cmo"),
// from the generated tulip_rs_ffi_state_ids.h. Never recompute it here.
var CmoID = uint32(C.C_INDICATOR_ID_CMO)

// CmoState is a live CMO streaming-state handle: the state returned by
// Indicator, resumable via Batch, closeable exactly once via Close.
type CmoState struct{ *tulip.State }

func cmoFreeState(h uintptr) { C.cmo_state_free(C.u2p(C.uintptr_t(h))) }

func wrapCmoState(h uintptr) *CmoState {
	return &CmoState{tulip.NewState(CmoID, h, cmoFreeState)}
}
...
// MinData returns the minimum number of bars CMO needs to produce output.
func (cmo) MinData(options []float64) uint64 {
	opts, keep := optionArg(options)
	n := uint64(C.cmo_min_data(opts))
	runtime.KeepAlive(keep)
	return n
}

// Indicator computes CMO over the given bars.
//
// Outputs: Rows is [cmo_line]. The returned CmoState is live
// streaming state — closing the Result does NOT free it (and vice versa).
func (cmo) Indicator(real []float64, options []float64, optionalOutputs []bool) (*tulip.Result, *CmoState, error) {
	if len(options) != CmoOptions {
		return nil, nil, fmt.Errorf("cmo: Indicator needs %d option(s) [period], got %d", CmoOptions, len(options))
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	if err != nil {
		return nil, nil, fmt.Errorf("cmo: %w", err)
	}
	defer release()
	...
	return res, wrapCmoState(uintptr(raw.state)), nil
}

// Batch continues the stream with new bars; no need to re-send history.
func (s *CmoState) Batch(real []float64, optionalOutputs []bool) (*tulip.Result, error) {
	if s.Closed() {
	...
	arr, release, err := tulip.MarshalSeries(series, len(real))
	if err != nil {
		return nil, fmt.Errorf("cmo: %w", err)
	}
	defer release()
	optArr, optRelease := tulip.MarshalBools(optionalOutputs)
	defer optRelease()
...
func (s *CmoState) Clone() (*CmoState, error) {
	c, err := s.State.Clone()
	if err != nil {
		return nil, err
	}
	return &CmoState{c}, nil
}
...
func (cmo) DeserializeState(blob []byte) (*CmoState, error) {
	st, err := tulip.DeserializeState(CmoID, blob, cmoFreeState)
	if err != nil {
		return nil, fmt.Errorf("cmo: %w", err)
	}
	return &CmoState{st}, nil
}

// SimdByAssets computes CMO for N assets in one CPU pass. Each asset
...
func (cmo) SimdByAssets(assets [][CmoInputs][]float64, options []float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(options) != CmoOptions {
		return nil, fmt.Errorf("cmo: SimdByAssets needs %d option(s) [period], got %d", CmoOptions, len(options))
	}
...
// SimdByOptions computes CMO for N option sets over one shared series of
// bars in one CPU pass. Each lane's resumable state lives in
// SimdResult.States.
func (cmo) SimdByOptions(real []float64, optionSets [][]float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(optionSets) == 0 {
		return nil, fmt.Errorf("cmo: no option sets")
	}
	for i, o := range optionSets {
		if len(o) != CmoOptions {
			return nil, fmt.Errorf("cmo: option set %d has %d values, need %d", i, len(o), CmoOptions)
		}
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	...
	defer release()
	sets := padSets(optionSets, CmoOptions)
