...
// Macd is the namespaced entry point for Moving Average Convergence Divergence.
type macd struct{}

var Macd = macd{}

// MacdInputs / MacdOptions: series/option arities, from the generated C
// defines (tulip_rs_ffi_counts.h) so they can never drift from the core.
const (
	MacdInputs  = int(C.MACD_INPUTS)  // real
	MacdOptions = int(C.MACD_OPTIONS) // fastperiod,slowperiod,signalperiod
)

// MacdID is the FFI indicator id for state serialisation: fnv1a32("macd"),
// from the generated tulip_rs_ffi_state_ids.h. Never recompute it here.
var MacdID = uint32(C.C_INDICATOR_ID_MACD)

// MacdState is a live MACD streaming-state handle: the state returned by
// Indicator, resumable via Batch, closeable exactly once via Close.
type MacdState struct{ *tulip.State }

func macdFreeState(h uintptr) { C.macd_state_free(C.u2p(C.uintptr_t(h))) }

func wrapMacdState(h uintptr) *MacdState {
	return &MacdState{tulip.NewState(MacdID, h, macdFreeState)}
}
...
// MinData returns the minimum number of bars MACD needs to produce output.
func (macd) MinData(options []float64) uint64 {
	opts, keep := optionArg(options)
	n := uint64(C.macd_min_data(opts))
	runtime.KeepAlive(keep)
	return n
}

// Indicator computes MACD over the given bars.
//
// Outputs: with optionalOutputs empty/nil, Rows is just [macd]; with two
// flags, Rows is [macd, short_ema, long_ema]. The returned MacdState is live
// streaming state — closing the Result does NOT free it (and vice versa).
func (macd) Indicator(real []float64, options []float64, optionalOutputs []bool) (*tulip.Result, *MacdState, error) {
	if len(options) != MacdOptions {
		return nil, nil, fmt.Errorf("macd: Indicator needs %d option(s) [fastperiod,slowperiod,signalperiod], got %d", MacdOptions, len(options))
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	if err != nil {
		return nil, nil, fmt.Errorf("macd: %w", err)
	}
	defer release()
	...
	return res, wrapMacdState(uintptr(raw.state)), nil
}

// Batch continues the stream with new bars; no need to re-send history.
func (s *MacdState) Batch(real []float64, optionalOutputs []bool) (*tulip.Result, error) {
	if s.Closed() {
...
func (s *MacdState) Clone() (*MacdState, error) {
	c, err := s.State.Clone()
	if err != nil {
		return nil, err
	}
	return &MacdState{c}, nil
}
...
func (macd) DeserializeState(blob []byte) (*MacdState, error) {
	st, err := tulip.DeserializeState(MacdID, blob, macdFreeState)
	if err != nil {
		return nil, fmt.Errorf("macd: %w", err)
	}
	return &MacdState{st}, nil
}

// SimdByAssets computes MACD for N assets in one CPU pass. Each asset
...
func (macd) SimdByAssets(assets [][MacdInputs][]float64, options []float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(options) != MacdOptions {
		return nil, fmt.Errorf("macd: SimdByAssets needs %d option(s) [fastperiod,slowperiod,signalperiod], got %d", MacdOptions, len(options))
	}
...
// SimdByOptions computes MACD for N option sets over one shared series of
// bars in one CPU pass. Each lane's resumable state lives in SimdResult.States.
func (macd) SimdByOptions(real []float64, optionSets [][]float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(optionSets) == 0 {
		return nil, fmt.Errorf("macd: no option sets")
	}
	for i, o := range optionSets {
		if len(o) != MacdOptions {
			return nil, fmt.Errorf("macd: option set %d has %d values, need %d", i, len(o), MacdOptions)
		}
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	...
	defer release()
	sets := padSets(optionSets, MacdOptions)
