...
// Mass is the namespaced entry point for Mass Index.
type mass struct{}

var Mass = mass{}

// MassInputs / MassOptions: series/option arities, from the generated C
// defines (tulip_rs_ffi_counts.h) so they can never drift from the core.
const (
	MassInputs  = int(C.MASS_INPUTS)  // high, low
	MassOptions = int(C.MASS_OPTIONS) // period
)

// MassID is the FFI indicator id for state serialisation: fnv1a32("mass"),
// from the generated tulip_rs_ffi_state_ids.h. Never recompute it here.
var MassID = uint32(C.C_INDICATOR_ID_MASS)

// MassState is a live Mass Index streaming-state handle:
// the state returned by Indicator, resumable via Batch, closeable exactly once via Close.
type MassState struct{ *tulip.State }

func massFreeState(h uintptr) { C.mass_state_free(C.u2p(C.uintptr_t(h))) }

func wrapMassState(h uintptr) *MassState {
	return &MassState{tulip.NewState(MassID, h, massFreeState)}
}
...
// MinData returns the minimum number of bars Mass needs to produce output.
func (mass) MinData(options []float64) uint64 {
	opts, keep := optionArg(options)
	n := uint64(C.mass_min_data(opts))
	runtime.KeepAlive(keep)
	return n
}

// Indicator computes Mass Index over the given bars.
//
// Outputs: Rows is just [mass] (no optional outputs). The returned MassState
// is live streaming state — closing the Result does NOT free it (and vice versa).
func (mass) Indicator(high, low []float64, options []float64, optionalOutputs []bool) (*tulip.Result, *MassState, error) {
	if len(options) != MassOptions {
		return nil, nil, fmt.Errorf("mass: Indicator needs %d option(s) [period], got %d", MassOptions, len(options))
	}
	series := [][]float64{high, low}
	arr, release, err := tulip.MarshalSeries(series, len(high))
	if err != nil {
		return nil, nil, fmt.Errorf("mass: %w", err)
	}
	defer release()
	...
	return res, wrapMassState(uintptr(raw.state)), nil
}

// Batch continues the stream with new bars; no need to re-send history.
func (s *MassState) Batch(high, low []float64, optionalOutputs []bool) (*tulip.Result, error) {
	if s.Closed() {
...
func (s *MassState) Clone() (*MassState, error) {
	c, err := s.State.Clone()
	if err != nil {
		return nil, err
	}
	return &MassState{c}, nil
}
...
func (mass) DeserializeState(blob []byte) (*MassState, error) {
	st, err := tulip.DeserializeState(MassID, blob, massFreeState)
	if err != nil {
		return nil, fmt.Errorf("mass: %w", err)
	}
	return &MassState{st}, nil
}

// SimdByAssets computes Mass Index for N assets in one CPU pass. Each asset
...
func (mass) SimdByAssets(assets [][MassInputs][]float64, options []float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(options) != MassOptions {
		return nil, fmt.Errorf("mass: SimdByAssets needs %d option(s) [period], got %d", MassOptions, len(options))
	}
...
// SimdByOptions computes Mass Index for N option sets over one shared series of
// bars in one CPU pass. Each lane's resumable state lives in SimdResult.States.
func (mass) SimdByOptions(high, low []float64, optionSets [][]float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(optionSets) == 0 {
		return nil, fmt.Errorf("mass: no option sets")
	}
	for i, o := range optionSets {
		if len(o) != MassOptions {
			return nil, fmt.Errorf("mass: option set %d has %d values, need %d", i, len(o), MassOptions)
		}
	}
	series := [][]float64{high, low}
	arr, release, err := tulip.MarshalSeries(series, len(high))
	...
	defer release()
	sets := padSets(optionSets, MassOptions)
