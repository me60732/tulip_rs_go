...
// Vhf is the namespaced entry point for the Vertical Horizontal Filter.
type vhf struct{}

var Vhf = vhf{}

// VhfInputs / VhfOptions: series/option arities, from the generated C
// defines (tulip_rs_ffi_counts.h) so they can never drift from the core.
const (
	VhfInputs  = int(C.VHF_INPUTS)  // real
	VhfOptions = int(C.VHF_OPTIONS) // period
)

// VhfID is the FFI indicator id for state serialisation: fnv1a32("vhf"),
// from the generated tulip_rs_ffi_state_ids.h. Never recompute it here.
var VhfID = uint32(C.C_INDICATOR_ID_VHF)

// VhfState is a live VHF streaming-state handle: the state returned by
// Indicator, resumable via Batch, closeable exactly once via Close.
type VhfState struct{ *tulip.State }

func vhfFreeState(h uintptr) { C.vhf_state_free(C.u2p(C.uintptr_t(h))) }

func wrapVhfState(h uintptr) *VhfState {
	return &VhfState{tulip.NewState(VhfID, h, vhfFreeState)}
}
...
// MinData returns the minimum number of bars VHF needs to produce output.
func (vhf) MinData(options []float64) uint64 {
	opts, keep := optionArg(options)
	n := uint64(C.vhf_min_data(opts))
	runtime.KeepAlive(keep)
	return n
}

// Indicator computes VHF over the given bars.
//
// Outputs: with optionalOutputs empty/nil, Rows is just [vhf]; with four
// flags, Rows is [vhf, short_sma, long_sma, short_stddev, long_stddev]. The returned VhfState is live
// streaming state — closing the Result does NOT free it (and vice versa).
func (vhf) Indicator(real []float64, options []float64, optionalOutputs []bool) (*tulip.Result, *VhfState, error) {
	if len(options) != VhfOptions {
		return nil, nil, fmt.Errorf("vhf: Indicator needs %d option(s) [period], got %d", VhfOptions, len(options))
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	if err != nil {
		return nil, nil, fmt.Errorf("vhf: %w", err)
	}
	defer release()
	...
	return res, wrapVhfState(uintptr(raw.state)), nil
}

// Batch continues the stream with new bars; no need to re-send history.
func (s *VhfState) Batch(real []float64, optionalOutputs []bool) (*tulip.Result, error) {
	if s.Closed() {
	...
	arr, release, err := tulip.MarshalSeries(series, len(real))
	if err != nil {
		return nil, fmt.Errorf("vhf: %w", err)
	}
	defer release()
	optArr, optRelease := tulip.MarshalBools(optionalOutputs)
	defer optRelease()
...
func (s *VhfState) Clone() (*VhfState, error) {
	c, err := s.State.Clone()
	if err != nil {
		return nil, err
	}
	return &VhfState{c}, nil
}
...
func (vhf) DeserializeState(blob []byte) (*VhfState, error) {
	st, err := tulip.DeserializeState(VhfID, blob, vhfFreeState)
	if err != nil {
		return nil, fmt.Errorf("vhf: %w", err)
	}
	return &VhfState{st}, nil
}

// SimdByAssets computes VHF for N assets in one CPU pass. Each asset
...
func (vhf) SimdByAssets(assets [][VhfInputs][]float64, options []float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(options) != VhfOptions {
		return nil, fmt.Errorf("vhf: SimdByAssets needs %d option(s) [period], got %d", VhfOptions, len(options))
	}
...
// SimdByOptions computes VHF for N option sets over one shared series of
// bars in one CPU pass. Each lane's resumable state lives in
// SimdResult.States.
func (vhf) SimdByOptions(real []float64, optionSets [][]float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(optionSets) == 0 {
		return nil, fmt.Errorf("vhf: no option sets")
	}
	for i, o := range optionSets {
		if len(o) != VhfOptions {
			return nil, fmt.Errorf("vhf: option set %d has %d values, need %d", i, len(o), VhfOptions)
		}
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	...
	defer release()
	sets := padSets(optionSets, VhfOptions)
