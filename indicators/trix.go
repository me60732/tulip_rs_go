...
// Trix is the namespaced entry point for the Triple Exponential Average.
type trix struct{}

var Trix = trix{}

// TrixInputs / TrixOptions: series/option arities, from the generated C
// defines (tulip_rs_ffi_counts.h) so they can never drift from the core.
const (
	TrixInputs  = int(C.TRIX_INPUTS)  // real
	TrixOptions = int(C.TRIX_OPTIONS) // period
)

// TrixID is the FFI indicator id for state serialisation: fnv1a32("trix"),
// from the generated tulip_rs_ffi_state_ids.h. Never recompute it here.
var TrixID = uint32(C.C_INDICATOR_ID_TRIX)

// TrixState is a live TRIX streaming-state handle: the state returned by
// Indicator, resumable via Batch, closeable exactly once via Close.
type TrixState struct{ *tulip.State }

func trixFreeState(h uintptr) { C.trix_state_free(C.u2p(C.uintptr_t(h))) }

func wrapTrixState(h uintptr) *TrixState {
	return &TrixState{tulip.NewState(TrixID, h, trixFreeState)}
}
...
// MinData returns the minimum number of bars TRIX needs to produce output.
func (trix) MinData(options []float64) uint64 {
	opts, keep := optionArg(options)
	n := uint64(C.trix_min_data(opts))
	runtime.KeepAlive(keep)
	return n
}

// Indicator computes TRIX over the given bars.
//
// Outputs: with optionalOutputs empty/nil, Rows is just [trix]; with three
// flags, Rows is [trix, tema, dema, ema]. The returned TrixState is live
// streaming state — closing the Result does NOT free it (and vice versa).
func (trix) Indicator(real []float64, options []float64, optionalOutputs []bool) (*tulip.Result, *TrixState, error) {
	if len(options) != TrixOptions {
		return nil, nil, fmt.Errorf("trix: Indicator needs %d option(s) [period], got %d", TrixOptions, len(options))
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	if err != nil {
		return nil, nil, fmt.Errorf("trix: %w", err)
	}
	defer release()
	...
	return res, wrapTrixState(uintptr(raw.state)), nil
}

// Batch continues the stream with new bars; no need to re-send history.
func (s *TrixState) Batch(real []float64, optionalOutputs []bool) (*tulip.Result, error) {
	if s.Closed() {
	...
	arr, release, err := tulip.MarshalSeries(series, len(real))
	if err != nil {
		return nil, fmt.Errorf("trix: %w", err)
	}
	defer release()
	optArr, optRelease := tulip.MarshalBools(optionalOutputs)
	defer optRelease()
...
func (s *TrixState) Clone() (*TrixState, error) {
	c, err := s.State.Clone()
	if err != nil {
		return nil, err
	}
	return &TrixState{c}, nil
}
...
func (trix) DeserializeState(blob []byte) (*TrixState, error) {
	st, err := tulip.DeserializeState(TrixID, blob, trixFreeState)
	if err != nil {
		return nil, fmt.Errorf("trix: %w", err)
	}
	return &TrixState{st}, nil
}

// SimdByAssets computes TRIX for N assets in one CPU pass. Each asset
...
func (trix) SimdByAssets(assets [][TrixInputs][]float64, options []float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(options) != TrixOptions {
		return nil, fmt.Errorf("trix: SimdByAssets needs %d option(s) [period], got %d", TrixOptions, len(options))
	}
...
// SimdByOptions computes TRIX for N option sets over one shared series of
// bars in one CPU pass. Each lane's resumable state lives in
// SimdResult.States.
func (trix) SimdByOptions(real []float64, optionSets [][]float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(optionSets) == 0 {
		return nil, fmt.Errorf("trix: no option sets")
	}
	for i, o := range optionSets {
		if len(o) != TrixOptions {
			return nil, fmt.Errorf("trix: option set %d has %d values, need %d", i, len(o), TrixOptions)
		}
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	...
	defer release()
	sets := padSets(optionSets, TrixOptions)
