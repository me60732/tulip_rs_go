...
// Adxr is the namespaced entry point for the Average Directional Index Rating.
type adxr struct{}

var Adxr = adxr{}

// AdxrInputs / AdxrOptions: series/option arities, from the generated C
// defines (tulip_rs_ffi_counts.h) so they can never drift from the core.
const (
	AdxrInputs  = int(C.ADXR_INPUTS)  // high, low, close
	AdxrOptions = int(C.ADXR_OPTIONS) // period
)

// AdxrID is the FFI indicator id for state serialisation: fnv1a32("adxr"),
// from the generated tulip_rs_ffi_state_ids.h. Never recompute it here.
var AdxrID = uint32(C.C_INDICATOR_ID_ADXR)

// AdxrState is a live ADXR streaming-state handle: the state returned by
// Indicator, resumable via Batch, closeable exactly once via Close.
type AdxrState struct{ *tulip.State }

func adxrFreeState(h uintptr) { C.adxr_state_free(C.u2p(C.uintptr_t(h))) }

func wrapAdxrState(h uintptr) *AdxrState {
	return &AdxrState{tulip.NewState(AdxrID, h, adxrFreeState)}
}
...
// MinData returns the minimum number of bars ADXR needs to produce output.
func (adxr) MinData(options []float64) uint64 {
	opts, keep := optionArg(options)
	n := uint64(C.adxr_min_data(opts))
	runtime.KeepAlive(keep)
	return n
}

// Indicator computes ADXR over the given bars.
//
// Outputs: with optionalOutputs empty/nil, Rows is just [adxr].
func (adxr) Indicator(high, low, close []float64, options []float64, optionalOutputs []bool) (*tulip.Result, *AdxrState, error) {
	if len(options) != AdxrOptions {
		return nil, nil, fmt.Errorf("adxr: Indicator needs %d option(s) [period], got %d", AdxrOptions, len(options))
	}
	series := [][]float64{high, low, close}
	arr, release, err := tulip.MarshalSeries(series, len(high))
	if err != nil {
		return nil, nil, fmt.Errorf("adxr: %w", err)
	}
	defer release()
	...
	return res, wrapAdxrState(uintptr(raw.state)), nil
}

// Batch continues the stream with new bars; no need to re-send history.
func (s *AdxrState) Batch(high, low, close []float64, optionalOutputs []bool) (*tulip.Result, error) {
	if s.Closed() {
	...
	arr, release, err := tulip.MarshalSeries(series, len(high))
	if err != nil {
		return nil, fmt.Errorf("adxr: %w", err)
	}
	defer release()
	optArr, optRelease := tulip.MarshalBools(optionalOutputs)
	defer optRelease()
...
func (s *AdxrState) Clone() (*AdxrState, error) {
	c, err := s.State.Clone()
	if err != nil {
		return nil, err
	}
	return &AdxrState{c}, nil
}
...
func (adxr) DeserializeState(blob []byte) (*AdxrState, error) {
	st, err := tulip.DeserializeState(AdxrID, blob, adxrFreeState)
	if err != nil {
		return nil, fmt.Errorf("adxr: %w", err)
	}
	return &AdxrState{st}, nil
}

// SimdByAssets computes ADXR for N assets in one CPU pass. Each asset
...
func (adxr) SimdByAssets(assets [][AdxrInputs][]float64, options []float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(options) != AdxrOptions {
		return nil, fmt.Errorf("adxr: SimdByAssets needs %d option(s) [period], got %d", AdxrOptions, len(options))
	}
...
// SimdByOptions computes ADXR for N option sets over one shared series of
// bars in one CPU pass. Each lane's resumable state lives in
// SimdResult.States.
func (adxr) SimdByOptions(high, low, close []float64, optionSets [][]float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(optionSets) == 0 {
		return nil, fmt.Errorf("adxr: no option sets")
	}
	for i, o := range optionSets {
		if len(o) != AdxrOptions {
			return nil, fmt.Errorf("adxr: option set %d has %d values, need %d", i, len(o), AdxrOptions)
		}
	}
	series := [][]float64{high, low, close}
	arr, release, err := tulip.MarshalSeries(series, len(high))
	...
	defer release()
	sets := padSets(optionSets, AdxrOptions)
