...
// Max is the namespaced entry point for the Maximum indicator.
type max struct{}

var Max = max{}

// MaxInputs / MaxOptions: series/option arities, from the generated C
// defines (tulip_rs_ffi_counts.h) so they can never drift from the core.
const (
	MaxInputs  = int(C.MAX_INPUTS)  // real
	MaxOptions = int(C.MAX_OPTIONS) // period
)

// MaxID is the FFI indicator id for state serialisation: fnv1a32("max"),
// from the generated tulip_rs_ffi_state_ids.h. Never recompute it here.
var MaxID = uint32(C.C_INDICATOR_ID_MAX)

// MaxState is a live MAX streaming-state handle: the state returned by
// Indicator, resumable via Batch, closeable exactly once via Close.
type MaxState struct{ *tulip.State }

func maxFreeState(h uintptr) { C.max_state_free(C.u2p(C.uintptr_t(h))) }

func wrapMaxState(h uintptr) *MaxState {
	return &MaxState{tulip.NewState(MaxID, h, maxFreeState)}
}
...
// MinData returns the minimum number of bars MAX needs to produce output.
func (max) MinData(options []float64) uint64 {
	opts, keep := optionArg(options)
	n := uint64(C.max_min_data(opts))
	runtime.KeepAlive(keep)
	return n
}

// Indicator computes MAX over the given bars.
//
// Outputs: with optionalOutputs empty/nil, Rows is just [max]; there are no
// optional outputs for this indicator.
func (max) Indicator(real []float64, options []float64, optionalOutputs []bool) (*tulip.Result, *MaxState, error) {
	if len(options) != MaxOptions {
		return nil, nil, fmt.Errorf("max: Indicator needs %d option(s) [period], got %d", MaxOptions, len(options))
	}
	series := [][]float64{real}
	...
	return res, wrapMaxState(uintptr(raw.state)), nil
}

// Batch continues the stream with new bars; no need to re-send history.
func (s *MaxState) Batch(real []float64) (*tulip.Result, error) {
	if s.Closed() {
...
func (s *MaxState) Clone() (*MaxState, error) {
	c, err := s.State.Clone()
	if err != nil {
		return nil, err
	}
	return &MaxState{c}, nil
}
...
func (max) DeserializeState(blob []byte) (*MaxState, error) {
	st, err := tulip.DeserializeState(MaxID, blob, maxFreeState)
	if err != nil {
		return nil, fmt.Errorf("max: %w", err)
	}
	return &MaxState{st}, nil
}

// SimdByAssets computes MAX for N assets in one CPU pass. Each asset
// carries the same series (real) of equal length; all lanes share one options set.
func (max) SimdByAssets(assets [][MaxInputs][]float64, options []float64) (*tulip.SimdResult, error) {
	if len(options) != MaxOptions {
		return nil, fmt.Errorf("max: SimdByAssets needs %d option(s) [period], got %d", MaxOptions, len(options))
	}
...
// SimdByOptions computes MAX for N option sets over one shared series of
// bars in one CPU pass. Each lane's resumable state lives in
// SimdResult.States.
func (max) SimdByOptions(real []float64, optionSets [][]float64) (*tulip.SimdResult, error) {
	if len(optionSets) == 0 {
		return nil, fmt.Errorf("max: no option sets")
	}
	for i, o := range optionSets {
		if len(o) != MaxOptions {
			return nil, fmt.Errorf("max: option set %d has %d values, need %d", i, len(o), MaxOptions)
		}
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	...
	defer release()
	sets := padSets(optionSets, MaxOptions)
