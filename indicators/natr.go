...
// Natr is the namespaced entry point for the Normalized Average True Range indicator.
type natr struct{}

var Natr = natr{}

// NatrInputs / NatrOptions: series/option arities, from the generated C
// defines (tulip_rs_ffi_counts.h) so they can never drift from the core.
const (
	NatrInputs  = int(C.NATR_INPUTS)  // high, low, close
	NatrOptions = int(C.NATR_OPTIONS) // period
)

// NatrID is the FFI indicator id for state serialisation: fnv1a32("natr"),
// from the generated tulip_rs_ffi_state_ids.h. Never recompute it here.
var NatrID = uint32(C.C_INDICATOR_ID_NATR)

// NatrState is a live NATR streaming-state handle: the state returned by
// Indicator, resumable via Batch, closeable exactly once via Close.
type NatrState struct{ *tulip.State }

func natrFreeState(h uintptr) { C.natr_state_free(C.u2p(C.uintptr_t(h))) }

func wrapNatrState(h uintptr) *NatrState {
	return &NatrState{tulip.NewState(NatrID, h, natrFreeState)}
}
...
// MinData returns the minimum number of bars NATR needs to produce output.
func (natr) MinData(options []float64) uint64 {
	opts, keep := optionArg(options)
	n := uint64(C.natr_min_data(opts))
	runtime.KeepAlive(keep)
	return n
}

// Indicator computes NATR over the given bars.
//
// Outputs: with optionalOutputs empty/nil, Rows is just [natr]; with two
// flags enabled, Rows is [natr, atr, tr]. The returned NatrState is live
// streaming state — closing the Result does NOT free it (and vice versa).
func (natr) Indicator(high, low, close []float64, options []float64, optionalOutputs []bool) (*tulip.Result, *NatrState, error) {
	if len(options) != NatrOptions {
		return nil, nil, fmt.Errorf("natr: Indicator needs %d option(s) [period], got %d", NatrOptions, len(options))
	}
	series := [][]float64{high, low, close}
	arr, release, err := tulip.MarshalSeries(series, len(high))
	if err != nil {
		return nil, nil, fmt.Errorf("natr: %w", err)
	}
	defer release()
	...
	return res, wrapNatrState(uintptr(raw.state)), nil
}

// Batch continues the stream with new bars; no need to re-send history.
func (s *NatrState) Batch(high, low, close []float64, optionalOutputs []bool) (*tulip.Result, error) {
	if s.Closed() {
	...
	arr, release, err := tulip.MarshalSeries(series, len(high))
	if err != nil {
		return nil, fmt.Errorf("natr: %w", err)
	}
	defer release()
	optArr, optRelease := tulip.MarshalBools(optionalOutputs)
	defer optRelease()
...
func (s *NatrState) Clone() (*NatrState, error) {
	c, err := s.State.Clone()
	if err != nil {
		return nil, err
	}
	return &NatrState{c}, nil
}
...
func (natr) DeserializeState(blob []byte) (*NatrState, error) {
	st, err := tulip.DeserializeState(NatrID, blob, natrFreeState)
	if err != nil {
		return nil, fmt.Errorf("natr: %w", err)
	}
	return &NatrState{st}, nil
}

// SimdByAssets computes NATR for N assets in one CPU pass. Each asset
...
func (natr) SimdByAssets(assets [][NatrInputs][]float64, options []float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(options) != NatrOptions {
		return nil, fmt.Errorf("natr: SimdByAssets needs %d option(s) [period], got %d", NatrOptions, len(options))
	}
...
// SimdByOptions computes NATR for N option sets over one shared series of
// bars in one CPU pass. Each lane's resumable state lives in
// SimdResult.States.
func (natr) SimdByOptions(high, low, close []float64, optionSets [][]float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(optionSets) == 0 {
		return nil, fmt.Errorf("natr: no option sets")
	}
	for i, o := range optionSets {
		if len(o) != NatrOptions {
			return nil, fmt.Errorf("natr: option set %d has %d values, need %d", i, len(o), NatrOptions)
		}
	}
	series := [][]float64{high, low, close}
	arr, release, err := tulip.MarshalSeries(series, len(high))
	...
	defer release()
	sets := padSets(optionSets, NatrOptions)
