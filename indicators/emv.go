package indicators

/*
#cgo CFLAGS: -I${SRCDIR}/../ffi/include
#include <stdbool.h>
#include <stdint.h>
// cgo prologue defines a `CBytes` helper; rename the FFI typedef to avoid
// the clash (must match the rename in every cgo file of this package and in
// the tulip package's preamble).
#define CBytes TulipBytes
#include "tulip_rs_ffi.h"

static inline void *u2p(uintptr_t p) { return (void *)p; }
*/
import "C"

import (
	"fmt"
	"runtime"
	"unsafe"

	"github.com/me60732/tulip_rs_go/tulip"
)

// Emv is the namespaced entry point for the Ease of Movement.
type emv struct{}

var Emv = emv{}

// EmvInputs / EmvOptions: series/option arities, from the generated C
// defines (tulip_rs_ffi_counts.h) so they can never drift from the core.
const (
	EmvInputs  = int(C.EMV_INPUTS)  // high, low, volume
	EmvOptions = int(C.EMV_OPTIONS) // no options
)

// EmvID is the FFI indicator id for state serialisation: fnv1a32("emv"),
// from the generated tulip_rs_ffi_state_ids.h. Never recompute it here.
var EmvID = uint32(C.C_INDICATOR_ID_EMV)

// EmvState is a live EMV streaming-state handle: the state returned by
// Indicator, resumable via Batch, closeable exactly once via Close.
type EmvState struct{ *tulip.State }

func emvFreeState(h uintptr) { C.emv_state_free(C.u2p(C.uintptr_t(h))) }

func wrapEmvState(h uintptr) *EmvState {
	return &EmvState{tulip.NewState(EmvID, h, emvFreeState)}
}

// Info returns the indicator's metadata as pure Go values. The underlying
// C strings are process-lifetime; they are copied and never freed.
func (emv) Info() tulip.Info {
	ci := C.emv_info()
	return tulip.NewInfo(
		int32(ci.indicator_type),
		C.GoString(ci.name),
		C.GoString(ci.full_name),
		tulip.CopyStringArray(uintptr(unsafe.Pointer(ci.inputs.ptr)), int(ci.inputs.len)),
		tulip.CopyStringArray(uintptr(unsafe.Pointer(ci.options.ptr)), int(ci.options.len)),
		tulip.CopyStringArray(uintptr(unsafe.Pointer(ci.outputs.ptr)), int(ci.outputs.len)),
		tulip.CopyStringArray(uintptr(unsafe.Pointer(ci.optional_outputs.ptr)), int(ci.optional_outputs.len)),
		tulip.CopyDisplayGroups(uintptr(unsafe.Pointer(ci.display_groups.ptr)), int(ci.display_groups.len)),
	)
}

// MinData returns the minimum number of bars EMV needs to produce output.
func (emv) MinData(options []float64) uint64 {
	opts, keep := optionArg(options)
	n := uint64(C.emv_min_data(opts))
	runtime.KeepAlive(keep)
	return n
}

// Indicator computes EMV over the given bars.
//
// Outputs: with optionalOutputs empty/nil, Rows is just [emv]; with the
// "medprice" flag enabled, Rows is [emv, medprice].
func (emv) Indicator(high, low, volume []float64, options []float64, optionalOutputs []bool) (*tulip.Result, *EmvState, error) {
	series := [][]float64{high, low, volume}
	arr, release, err := tulip.MarshalSeries(series, len(high))
	if err != nil {
		return nil, nil, fmt.Errorf("emv: %w", err)
	}
	defer release()
	optArr, optRelease := tulip.MarshalBools(optionalOutputs)
	defer optRelease()

	raw := C.emv_indicator(
		(**C.double)(arr),
		C.size_t(len(high)),
		nil,
		(*C.bool)(optArr),
		C.size_t(len(optionalOutputs)),
	)
	res, err := tulip.NewResult(
		int32(raw.error),
		unsafe.Pointer(raw.outputs),
		unsafe.Pointer(raw.output_lens),
		int(raw.num_outputs),
		uintptr(raw.state),
	)
	runtime.KeepAlive(series)
	if err != nil {
		return nil, nil, fmt.Errorf("emv: %w", err)
	}
	return res, wrapEmvState(uintptr(raw.state)), nil
}

// Batch continues the stream with new bars; no need to re-send history.
func (s *EmvState) Batch(high, low, volume []float64, optionalOutputs []bool) (*tulip.Result, error) {
	if s.Closed() {
		return nil, tulip.ErrClosed
	}
	series := [][]float64{high, low, volume}
	arr, release, err := tulip.MarshalSeries(series, len(high))
	if err != nil {
		return nil, fmt.Errorf("emv: %w", err)
	}
	defer release()
	optArr, optRelease := tulip.MarshalBools(optionalOutputs)
	defer optRelease()

	raw := C.emv_batch(
		s.Handle(),
		(**C.double)(arr),
		C.size_t(len(high)),
		(*C.bool)(optArr),
		C.size_t(len(optionalOutputs)),
	)
	res, err := tulip.NewBatchResult(
		int32(raw.error),
		unsafe.Pointer(raw.outputs),
		unsafe.Pointer(raw.output_lens),
		int(raw.num_outputs),
	)
	runtime.KeepAlive(series)
	if err != nil {
		return nil, fmt.Errorf("emv: %w", err)
	}
	return res, nil
}

// Clone returns an independent deep copy of the state (no serde involved).
func (s *EmvState) Clone() (*EmvState, error) {
	c, err := s.State.Clone()
	if err != nil {
		return nil, err
	}
	return &EmvState{c}, nil
}

// DeserializeState restores a streaming state from a TRFS blob produced by
// State.Serialize (from this or any other tulip_rs_ffi binding).
func (emv) DeserializeState(blob []byte) (*EmvState, error) {
	st, err := tulip.DeserializeState(EmvID, blob, emvFreeState)
	if err != nil {
		return nil, fmt.Errorf("emv: %w", err)
	}
	return &EmvState{st}, nil
}

// SimdByAssets computes EMV for N assets in one CPU pass. Each asset
// carries the same series (high, low, volume) of equal length; all lanes
// share no options set (zero-option indicator).
func (emv) SimdByAssets(assets [][EmvInputs][]float64, options []float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	groups := make([][][]float64, len(assets))
	for i, a := range assets {
		groups[i] = [][]float64{a[0], a[1], a[2]}
	}
	outer, release, err := tulip.MarshalSimdInputs(groups, len(assets[0][0]))
	if err != nil {
		return nil, fmt.Errorf("emv: %w", err)
	}
	defer release()
	optArr, optRelease := tulip.MarshalBools(optionalOutputs)
	defer optRelease()

	raw := C.emv_simd_by_assets(
		(***C.double)(outer),
		C.size_t(len(assets)),
		C.size_t(len(assets[0][0])),
		nil,
		(*C.bool)(optArr),
		C.size_t(len(optionalOutputs)),
	)
	res, err := tulip.NewSimdResult(
		int32(raw.error),
		unsafe.Pointer(raw.outputs),
		unsafe.Pointer(raw.output_lens),
		unsafe.Pointer(raw.states),
		int(raw.num_results),
		int(raw.num_outputs),
		EmvID, emvFreeState,
	)
	runtime.KeepAlive(assets)
	if err != nil {
		return nil, fmt.Errorf("emv: %w", err)
	}
	return res, nil
}
