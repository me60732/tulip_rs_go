...
// Fisher is the namespaced entry point for the Fisher Transform.
type fisher struct{}

var Fisher = fisher{}

// FisherInputs / FisherOptions: series/option arities, from the generated C
// defines (tulip_rs_ffi_counts.h) so they can never drift from the core.
const (
	FisherInputs  = int(C.FISHER_INPUTS)  // high, low
	FisherOptions = int(C.FISHER_OPTIONS) // period
)

// FisherID is the FFI indicator id for state serialisation: fnv1a32("fisher"),
// from the generated tulip_rs_ffi_state_ids.h. Never recompute it here.
var FisherID = uint32(C.C_INDICATOR_ID_FISHER)

// FisherState is a live Fisher Transform streaming-state handle: the state returned by
// Indicator, resumable via Batch, closeable exactly once via Close.
type FisherState struct{ *tulip.State }

func fisherFreeState(h uintptr) { C.fisher_state_free(C.u2p(C.uintptr_t(h))) }

func wrapFisherState(h uintptr) *FisherState {
	return &FisherState{tulip.NewState(FisherID, h, fisherFreeState)}
}
...
// MinData returns the minimum number of bars Fisher needs to produce output.
func (fisher) MinData(options []float64) uint64 {
	opts, keep := optionArg(options)
	n := uint64(C.fisher_min_data(opts))
	runtime.KeepAlive(keep)
	return n
}

// Indicator computes Fisher Transform over the given bars.
//
// Outputs: with optionalOutputs empty/nil, Rows is [fisher, fisher_signal].
func (fisher) Indicator(high, low []float64, options []float64, optionalOutputs []bool) (*tulip.Result, *FisherState, error) {
	if len(options) != FisherOptions {
		return nil, nil, fmt.Errorf("fisher: Indicator needs %d option(s) [period], got %d", FisherOptions, len(options))
	}
	series := [][]float64{high, low}
	arr, release, err := tulip.MarshalSeries(series, len(high))
	if err != nil {
		return nil, nil, fmt.Errorf("fisher: %w", err)
	}
	defer release()
	...
	return res, wrapFisherState(uintptr(raw.state)), nil
}

// Batch continues the stream with new bars; no need to re-send history.
func (s *FisherState) Batch(high, low []float64, optionalOutputs []bool) (*tulip.Result, error) {
	if s.Closed() {
...
func (s *FisherState) Clone() (*FisherState, error) {
	c, err := s.State.Clone()
	if err != nil {
		return nil, err
	}
	return &FisherState{c}, nil
}
...
func (fisher) DeserializeState(blob []byte) (*FisherState, error) {
	st, err := tulip.DeserializeState(FisherID, blob, fisherFreeState)
	if err != nil {
		return nil, fmt.Errorf("fisher: %w", err)
	}
	return &FisherState{st}, nil
}

// SimdByAssets computes Fisher Transform for N assets in one CPU pass. Each asset
...
func (fisher) SimdByAssets(assets [][FisherInputs][]float64, options []float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(options) != FisherOptions {
		return nil, fmt.Errorf("fisher: SimdByAssets needs %d option(s) [period], got %d", FisherOptions, len(options))
	}
...
// SimdByOptions computes Fisher Transform for N option sets over one shared series of
// bars in one CPU pass. Each lane's resumable state lives in
// SimdResult.States.
func (fisher) SimdByOptions(high, low []float64, optionSets [][]float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(optionSets) == 0 {
		return nil, fmt.Errorf("fisher: no option sets")
	}
	for i, o := range optionSets {
		if len(o) != FisherOptions {
			return nil, fmt.Errorf("fisher: option set %d has %d values, need %d", i, len(o), FisherOptions)
		}
	}
	series := [][]float64{high, low}
	arr, release, err := tulip.MarshalSeries(series, len(high))
	...
	defer release()
	sets := padSets(optionSets, FisherOptions)
