...
// Dx is the namespaced entry point for the Directional Index.
type dx struct{}

var Dx = dx{}

// DxInputs / DxOptions: series/option arities, from the generated C
// defines (tulip_rs_ffi_counts.h) so they can never drift from the core.
const (
	DxInputs  = int(C.DX_INPUTS)  // high, low, close
	DxOptions = int(C.DX_OPTIONS) // period
)

// DxID is the FFI indicator id for state serialisation: fnv1a32("dx"),
// from the generated tulip_rs_ffi_state_ids.h. Never recompute it here.
var DxID = uint32(C.C_INDICATOR_ID_DX)

// DxState is a live DX streaming-state handle: the state returned by
// Indicator, resumable via Batch, closeable exactly once via Close.
type DxState struct{ *tulip.State }

func dxFreeState(h uintptr) { C.dx_state_free(C.u2p(C.uintptr_t(h))) }

func wrapDxState(h uintptr) *DxState {
	return &DxState{tulip.NewState(DxID, h, dxFreeState)}
}
...
// MinData returns the minimum number of bars DX needs to produce output.
func (dx) MinData(options []float64) uint64 {
	opts, keep := optionArg(options)
	n := uint64(C.dx_min_data(opts))
	runtime.KeepAlive(keep)
	return n
}

// Indicator computes DX over the given bars.
//
// Outputs: with optionalOutputs empty/nil, Rows is just [dx]; with one
// flag, Rows is [dx]. The returned DxState is live
// streaming state — closing the Result does NOT free it (and vice versa).
func (dx) Indicator(high, low, close []float64, options []float64, optionalOutputs []bool) (*tulip.Result, *DxState, error) {
	if len(options) != DxOptions {
		return nil, nil, fmt.Errorf("dx: Indicator needs %d option(s) [period], got %d", DxOptions, len(options))
	}
	series := [][]float64{high, low, close}
	arr, release, err := tulip.MarshalSeries(series, len(high))
	if err != nil {
		return nil, nil, fmt.Errorf("dx: %w", err)
	}
	defer release()
	...
	return res, wrapDxState(uintptr(raw.state)), nil
}

// Batch continues the stream with new bars; no need to re-send history.
func (s *DxState) Batch(high, low, close []float64, optionalOutputs []bool) (*tulip.Result, error) {
	if s.Closed() {
	...
	arr, release, err := tulip.MarshalSeries(series, len(high))
	if err != nil {
		return nil, fmt.Errorf("dx: %w", err)
	}
	defer release()
	optArr, optRelease := tulip.MarshalBools(optionalOutputs)
	defer optRelease()
...
func (s *DxState) Clone() (*DxState, error) {
	c, err := s.State.Clone()
	if err != nil {
		return nil, err
	}
	return &DxState{c}, nil
}
...
func (dx) DeserializeState(blob []byte) (*DxState, error) {
	st, err := tulip.DeserializeState(DxID, blob, dxFreeState)
	if err != nil {
		return nil, fmt.Errorf("dx: %w", err)
	}
	return &DxState{st}, nil
}

// SimdByAssets computes DX for N assets in one CPU pass. Each asset
...
func (dx) SimdByAssets(assets [][DxInputs][]float64, options []float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(options) != DxOptions {
		return nil, fmt.Errorf("dx: SimdByAssets needs %d option(s) [period], got %d", DxOptions, len(options))
	}
...
// SimdByOptions computes DX for N option sets over one shared series of
// bars in one CPU pass. Each lane's resumable state lives in
// SimdResult.States.
func (dx) SimdByOptions(high, low, close []float64, optionSets [][]float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(optionSets) == 0 {
		return nil, fmt.Errorf("dx: no option sets")
	}
	for i, o := range optionSets {
		if len(o) != DxOptions {
			return nil, fmt.Errorf("dx: option set %d has %d values, need %d", i, len(o), DxOptions)
		}
	}
	series := [][]float64{high, low, close}
	arr, release, err := tulip.MarshalSeries(series, len(high))
	...
	defer release()
	sets := padSets(optionSets, DxOptions)
