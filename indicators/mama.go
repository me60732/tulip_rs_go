...
// Mama is the namespaced entry point for MESA Adaptive Moving Average.
type mama struct{}

var Mama = mama{}

// MamaInputs / MamaOptions: series/option arities, from the generated C
// defines (tulip_rs_ffi_counts.h) so they can never drift from the core.
const (
	MamaInputs  = int(C.MAMA_INPUTS)  // real
	MamaOptions = int(C.MAMA_OPTIONS) // fastlimit,slowlimit
)

// MamaID is the FFI indicator id for state serialisation: fnv1a32("mama"),
// from the generated tulip_rs_ffi_state_ids.h. Never recompute it here.
var MamaID = uint32(C.C_INDICATOR_ID_MAMA)

// MamaState is a live MESA Adaptive Moving Average streaming-state handle:
// the state returned by Indicator, resumable via Batch, closeable exactly once via Close.
type MamaState struct{ *tulip.State }

func mamaFreeState(h uintptr) { C.mama_state_free(C.u2p(C.uintptr_t(h))) }

func wrapMamaState(h uintptr) *MamaState {
	return &MamaState{tulip.NewState(MamaID, h, mamaFreeState)}
}
...
// MinData returns the minimum number of bars MAMA needs to produce output.
func (mama) MinData(options []float64) uint64 {
	opts, keep := optionArg(options)
	n := uint64(C.mama_min_data(opts))
	runtime.KeepAlive(keep)
	return n
}

// Indicator computes MAMA over the given bars.
//
// Outputs: with optionalOutputs empty/nil, Rows is just [mama]; with two
// flags, Rows is [mama, dc_period, alpha]. The returned MamaState is live
// streaming state — closing the Result does NOT free it (and vice versa).
func (mama) Indicator(real []float64, options []float64, optionalOutputs []bool) (*tulip.Result, *MamaState, error) {
	if len(options) != MamaOptions {
		return nil, nil, fmt.Errorf("mama: Indicator needs %d option(s) [fastlimit,slowlimit], got %d", MamaOptions, len(options))
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	if err != nil {
		return nil, nil, fmt.Errorf("mama: %w", err)
	}
	defer release()
	...
	return res, wrapMamaState(uintptr(raw.state)), nil
}

// Batch continues the stream with new bars; no need to re-send history.
func (s *MamaState) Batch(real []float64, optionalOutputs []bool) (*tulip.Result, error) {
	if s.Closed() {
	...
	arr, release, err := tulip.MarshalSeries(series, len(real))
	if err != nil {
		return nil, fmt.Errorf("mama: %w", err)
	}
	defer release()
	optArr, optRelease := tulip.MarshalBools(optionalOutputs)
	defer optRelease()
...
func (s *MamaState) Clone() (*MamaState, error) {
	c, err := s.State.Clone()
	if err != nil {
		return nil, err
	}
	return &MamaState{c}, nil
}
...
func (mama) DeserializeState(blob []byte) (*MamaState, error) {
	st, err := tulip.DeserializeState(MamaID, blob, mamaFreeState)
	if err != nil {
		return nil, fmt.Errorf("mama: %w", err)
	}
	return &MamaState{st}, nil
}

// SimdByAssets computes MAMA for N assets in one CPU pass. Each asset
...
func (mama) SimdByAssets(assets [][MamaInputs][]float64, options []float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(options) != MamaOptions {
		return nil, fmt.Errorf("mama: SimdByAssets needs %d option(s) [fastlimit,slowlimit], got %d", MamaOptions, len(options))
	}
...
// SimdByOptions computes MAMA for N option sets over one shared series of
// bars in one CPU pass. Each lane's resumable state lives in SimdResult.States.
func (mama) SimdByOptions(real []float64, optionSets [][]float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(optionSets) == 0 {
		return nil, fmt.Errorf("mama: no option sets")
	}
	for i, o := range optionSets {
		if len(o) != MamaOptions {
			return nil, fmt.Errorf("mama: option set %d has %d values, need %d", i, len(o), MamaOptions)
		}
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	...
	defer release()
	sets := padSets(optionSets, MamaOptions)
