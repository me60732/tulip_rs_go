...
// Md is the namespaced entry point for the Mean Deviation indicator.
type md struct{}

var Md = md{}

// MdInputs / MdOptions: series/option arities, from the generated C
// defines (tulip_rs_ffi_counts.h) so they can never drift from the core.
const (
	MdInputs  = int(C.MD_INPUTS)  // real
	MdOptions = int(C.MD_OPTIONS) // period
)

// MdID is the FFI indicator id for state serialisation: fnv1a32("md"),
// from the generated tulip_rs_ffi_state_ids.h. Never recompute it here.
var MdID = uint32(C.C_INDICATOR_ID_MD)

// MdState is a live MD streaming-state handle: the state returned by
// Indicator, resumable via Batch, closeable exactly once via Close.
type MdState struct{ *tulip.State }

func mdFreeState(h uintptr) { C.md_state_free(C.u2p(C.uintptr_t(h))) }

func wrapMdState(h uintptr) *MdState {
	return &MdState{tulip.NewState(MdID, h, mdFreeState)}
}
...
// MinData returns the minimum number of bars MD needs to produce output.
func (md) MinData(options []float64) uint64 {
	opts, keep := optionArg(options)
	n := uint64(C.md_min_data(opts))
	runtime.KeepAlive(keep)
	return n
}

// Indicator computes MD over the given bars.
//
// Outputs: with optionalOutputs empty/nil, Rows is just [md]; with one flag,
// Rows is [md, sma].
func (md) Indicator(real []float64, options []float64, optionalOutputs []bool) (*tulip.Result, *MdState, error) {
	if len(options) != MdOptions {
		return nil, nil, fmt.Errorf("md: Indicator needs %d option(s) [period], got %d", MdOptions, len(options))
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	if err != nil {
		return nil, nil, fmt.Errorf("md: %w", err)
	}
	defer release()
	...
	return res, wrapMdState(uintptr(raw.state)), nil
}

// Batch continues the stream with new bars; no need to re-send history.
func (s *MdState) Batch(real []float64, optionalOutputs []bool) (*tulip.Result, error) {
	if s.Closed() {
...
func (s *MdState) Clone() (*MdState, error) {
	c, err := s.State.Clone()
	if err != nil {
		return nil, err
	}
	return &MdState{c}, nil
}
...
func (md) DeserializeState(blob []byte) (*MdState, error) {
	st, err := tulip.DeserializeState(MdID, blob, mdFreeState)
	if err != nil {
		return nil, fmt.Errorf("md: %w", err)
	}
	return &MdState{st}, nil
}

// SimdByAssets computes MD for N assets in one CPU pass. Each asset
// carries the same series (real) of equal length; all lanes share one options set.
func (md) SimdByAssets(assets [][MdInputs][]float64, options []float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(options) != MdOptions {
		return nil, fmt.Errorf("md: SimdByAssets needs %d option(s) [period], got %d", MdOptions, len(options))
	}
...
// SimdByOptions computes MD for N option sets over one shared series of
// bars in one CPU pass. Each lane's resumable state lives in
// SimdResult.States.
func (md) SimdByOptions(real []float64, optionSets [][]float64, optionalOutputs []bool) (*tulip.SimdResult, error) {
	if len(optionSets) == 0 {
		return nil, fmt.Errorf("md: no option sets")
	}
	for i, o := range optionSets {
		if len(o) != MdOptions {
			return nil, fmt.Errorf("md: option set %d has %d values, need %d", i, len(o), MdOptions)
		}
	}
	series := [][]float64{real}
	arr, release, err := tulip.MarshalSeries(series, len(real))
	...
	defer release()
	sets := padSets(optionSets, MdOptions)
